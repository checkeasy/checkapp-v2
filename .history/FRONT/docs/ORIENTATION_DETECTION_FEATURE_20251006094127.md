...
### Rotation CSS de l'Interface
```css
/* Conteneur principal */
transform: rotate(90deg);
transform-origin: center center;

/* Ajustement des dimensions pour compenser la rotation */
width: 100vh;  /* Hauteur de la fenêtre devient la largeur */
height: 100vw; /* Largeur de la fenêtre devient la hauteur */
left: 50%;
top: 50%;
...
**Principe:**
1. On tourne le conteneur de 90° avec `rotate(90deg)`
2. On ajuste les dimensions (width/height inversées)
3. On centre le conteneur avec `translate(-50%, -50%)`
4. Résultat : L'interface est tournée mais reste dans le viewport

### Positionnement Adaptatif des Contrôles
```typescript
// Bouton de capture
className={`absolute z-10 ${
  isLandscapePhoto
    ? 'right-8 top-1/2 -translate-y-1/2'  // Droite
    : 'bottom-20 left-1/2 -translate-x-1/2'  // Bas
}`}

// Slider d'opacité
className={`absolute z-10 ${
  isLandscapePhoto
    ? 'left-4 top-1/2 -translate-y-1/2'  // Gauche
    : 'right-4 top-1/2 -translate-y-1/2'  // Droite
}`}
```
...
**Stratégie:** Utilisation de CSS standard pour garantir la compatibilité maximale sur tous les navigateurs modernes.
...
2. Vérifier que l'interface est normale (pas de rotation)
3. Vérifier que le bouton de capture est en bas
4. Vérifier que le slider d'opacité est à droite
5. Vérifier que la bannière n'est pas affichée
6. Capturer une photo

### Test 2 : Photo Paysage
1. Ouvrir le modal avec une photo paysage
2. Vérifier que l'interface est tournée de 90°
3. Vérifier que la bannière "Tournez votre téléphone" s'affiche
4. Vérifier que le bouton de capture est à droite
5. Vérifier que le slider d'opacité est à gauche
6. Tourner physiquement le téléphone de 90°
7. Vérifier que la photo s'aligne correctement
8. Capturer une photo

### Test 3 : Changement d'Orientation (Paysage → Portrait)
1. Ouvrir le modal avec une photo paysage
2. Vérifier que l'interface est tournée
3. Passer à une photo portrait (navigation)
4. Vérifier la transition fluide (500ms)
5. Vérifier que l'interface revient en position normale
6. Vérifier que la bannière disparaît
7. Vérifier que les contrôles sont repositionnés

### Test 4 : Changement d'Orientation (Portrait → Paysage)
1. Ouvrir le modal avec une photo portrait
2. Passer à une photo paysage
3. Vérifier la transition fluide
4. Vérifier que l'interface tourne de 90°
5. Vérifier que la bannière apparaît
6. Vérifier que les contrôles sont repositionnés

### Test 5 : Transitions Multiples
1. Naviguer entre plusieurs photos (portrait/paysage/portrait)
2. Vérifier que les transitions sont fluides
3. Vérifier qu'il n'y a pas de bugs visuels
4. Vérifier que les contrôles sont toujours accessibles
...
1. **Vibration haptique** : Vibrer quand l'utilisateur tourne son téléphone
2. **Animation de guidage** : Flèche animée montrant dans quelle direction tourner
3. **Détection automatique** : Détecter quand l'utilisateur a tourné son téléphone et masquer la bannière
4. **Prévisualisation** : Montrer un aperçu miniature de la photo dans la bonne orientation
5. **Statistiques** : Tracker combien de fois les utilisateurs doivent tourner leur téléphone
6. **Mode paysage inversé** : Supporter la rotation dans les deux sens (90° et -90°)
