...
**Mapping attendu :**
- **Info ménage** (onglet avec icône 🧹 Brush) → `travelerNote` ⚠️ INVERSÉ
- **Info pièce** (onglet avec icône 🏠 Home) → `infoEntrance` ou `cleanerNote`

**⚠️ IMPORTANT - Inversion des champs API :**
Dans l'API, les noms des champs ne correspondent PAS à leur contenu réel :
- `travelerNote` contient en réalité des **instructions de ménage** (ex: "CONSIGNE POUR MENAGE TU DOIS PRENDRE LE BALAIS")
- `cleanerNote` est souvent **vide**
- `infoEntrance` contient les **vraies informations d'accès** (ex: "OK C EST LES INFO D ENTREE")

**Donc le mapping est inversé pour afficher les bonnes infos dans les bons onglets :**
- `cleaningInfo` = `travelerNote` (contient les instructions de ménage)
- `roomInfo` = `infoEntrance` ou `cleanerNote` (contient les infos de la pièce)
...
**Changes:**
- ✅ **INVERSION** : `cleaningInfo` utilise maintenant `travelerNote` (qui contient les instructions de ménage)
- ✅ `roomInfo` utilise `infoEntrance` en priorité (qui contient les vraies infos d'accès)
- ✅ `cleanerNote` devient un fallback pour `roomInfo` (car souvent vide)
- ✅ Ajout d'un commentaire explicatif sur l'inversion
