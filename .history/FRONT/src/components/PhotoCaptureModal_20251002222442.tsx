...
export function PhotoCaptureModal({
  ...
  const handleShowComparison = useCallback((referenceId: string) => {
    ...
    if (capturedPhoto) {
      console.log('🔍 Affichage comparaison pour:', referenceId);
  ...
  const handleGhostOpacityChange = useCallback((e: React.MouseEvent<HTMLDivElement> | React.TouchEvent<HTMLDivElement>) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black">
      ...
      <div className="relative w-full h-full">
        ...
        {/* Contrôle d'opacité vertical */}
        <div className="absolute right-4 top-1/2 transform -translate-y-1/2 z-10">
          <div className="flex flex-col items-center gap-2 bg-black/60 rounded-full p-3 backdrop-blur-sm">
            <span className="text-white text-xs">👻</span>
            <div
              className="w-6 h-32 bg-white/30 rounded-full relative cursor-pointer"
              onClick={handleGhostOpacityChange}
            >
              <div
                className="absolute w-4 h-4 bg-white rounded-full border-2 border-white shadow-lg cursor-pointer transform -translate-x-1/2"
                style={{
                  left: '50%',
        ...
        <div className="absolute inset-0 bg-black z-40">
          ...
          <div className="absolute top-0 left-0 right-0 z-50 flex items-center justify-between p-4 bg-gradient-to-b from-black/80 to-transparent">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h2 className="text-white font-semibold">{pieceName} - Photo {currentRefIndex + 1}</h2>
                <p className="text-white/70 text-sm">Comparaison référence vs photo capturée</p>
              </div>
            </div>
          </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 h-full">
              ...
              <div className="bg-gray-900 rounded-2xl overflow-hidden">
                ...
                <div className="aspect-[3/4] relative">
                  <img
                    src={referencePhotos[currentRefIndex]?.url}
                    alt="Référence"
                    className="w-full h-full object-cover"
                  />
                </div>
