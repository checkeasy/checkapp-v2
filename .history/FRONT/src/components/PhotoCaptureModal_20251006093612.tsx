...
export function PhotoCaptureModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      // 🆕 Déverrouiller l'orientation quand le modal se ferme
      unlockOrientation();
    }
  }, [isOpen, unlockOrientation]);

  // 🆕 Adapter l'orientation de l'écran selon la photo de référence
  useEffect(() => {
    if (!isOpen || !referenceOrientation) return;

    // Verrouiller l'orientation selon la photo de référence
    const adaptOrientation = async () => {
      try {
        console.log('🔄 Adaptation de l\'orientation:', {
          referenceOrientation,
          currentOrientation
        });

        // Si l'orientation requise est différente de l'orientation actuelle,
        // on verrouille l'orientation pour forcer l'utilisateur à tourner son téléphone
        if (referenceOrientation !== currentOrientation) {
          await lockOrientation(referenceOrientation);
        }
      } catch (error) {
        console.error('❌ Erreur adaptation orientation:', error);
      }
    };
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black">
      {/* 🆕 Prompt d'orientation si nécessaire */}
      ...
      >
        ...
        <video
          ...
          // ✅ FIX: Suppression de l'effet miroir pour affichage normal
        />
        ...
        {currentRef && (
          <img
            ...
            style={{
              ...
              // ✅ FIX: Suppression de l'effet miroir pour correspondre à la vidéo
              mixBlendMode: 'normal'
            }}
          />
        )}
