...
export function PieceSelector({ 
  ...
  // 🎯 FIX: Appeler conditionnellement les hooks selon la route
  const isCheckIn = location.pathname.includes('checkin');

  // Appeler UNIQUEMENT le hook nécessaire pour éviter les erreurs
  const checkoutFlow = !isCheckIn ? useCheckoutFlow() : null;
  const checkinFlow = isCheckIn ? useCheckinFlow() : null;
