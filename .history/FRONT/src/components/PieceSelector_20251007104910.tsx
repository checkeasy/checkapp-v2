  ...
  return (
    <Drawer open={open} onOpenChange={setOpen}>
      <DrawerTrigger asChild>
        <Button 
          variant="ghost" 
          className="group h-auto px-4 py-3 text-sm font-medium text-foreground hover:text-primary transition-all duration-300 hover:scale-[1.02] bg-white/60 backdrop-blur-sm border border-white/40 rounded-lg shadow-card hover:shadow-elegant focus:ring-2 focus:ring-primary/20 max-w-[90vw]"
          onClick={handleSelectorOpen}
        >
          <div className="flex items-center gap-3 min-w-0">
            <Typography variant="section-subtitle" className="font-semibold truncate max-w-[200px] sm:max-w-[300px] md:max-w-[400px]">
        ...
        <div className="px-6 pb-6 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div className="space-y-4">
            {pieces.map((piece, pieceIndex) => {
              ...
              // En mode checkout, afficher la structure hiérarchique avec les tâches
              const shouldShowPieceOnly = isCheckIn || pieceTasks.length <= 1;

              // If in check-in mode OR piece has only one task, show just the piece name (clickable)
                    ...
                    <div className="flex items-center gap-3 w-full">
                      ...
                      <div className="flex-1 text-left min-w-0">
                        <div className="font-bold text-base break-words whitespace-normal">
                         ...
                         {/* 🎯 FIX: En check-in, afficher le nombre de tâches au lieu du label */}
                         <div className={`text-sm break-words whitespace-normal ${
                           isActivePiece ? 'text-primary-foreground/80' : 'text-muted-foreground'
                         }`}>
                           {isCheckIn
                             ? `${progress.completed}/${progress.total} tâche${progress.total > 1 ? 's' : ''}`
                      ...
                      <div className="flex-shrink-0">
                        ...
                        >
                          {isCompleted ? "Validée" : hasProgress ? "En cours" : "À faire"}
                        </Badge>
                      </div>
                    </div>
                  </Button>
