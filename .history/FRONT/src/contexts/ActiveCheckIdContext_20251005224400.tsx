...
export const ActiveCheckIdProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
   ...
   * 🎯 FIX: Ajout du logementId pour isolation par logement
   */
  const createNewCheckId = useCallback(async (
    ...
    try {
      // 🎯 FIX: Extraire le logementId depuis parcoursInfo
      // Le logement peut être dans parcoursInfo.logement ou extrait de parcoursInfo.id
      const logementId = parcoursInfo.logement || parcoursInfo.id;

      // ✅ Créer la session via checkSessionManager (IndexedDB)
      // 🎯 CORRECTION: Passer userInfo, parcoursInfo ET logementId
      const idbSession = await checkSessionManager.createCheckSession(
        ...
        logementId // 🎯 FIX: Passer le logementId
      );
      ...
      console.log('✅ CheckID créé et activé (IndexedDB):', { checkId, logementId });
      return checkId;
    } catch (error) {
