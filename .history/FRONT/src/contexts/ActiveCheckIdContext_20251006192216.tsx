...
export const ActiveCheckIdProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [currentCheckId, setCurrentCheckId] = useState<string | null>(() => {
    // 🎯 FIX CRITIQUE: Charger le CheckID depuis localStorage OU URL
    try {
      // 1. Essayer URL d'abord (priorité absolue)
      ...
      // 2. Essayer localStorage comme fallback
      const stored = localStorage.getItem(STORAGE_KEY_ACTIVE);
      if (stored) {
        // 🎯 IMPORTANT: Si on a un parcoursId dans l'URL mais un checkId dans localStorage,
        // il faut vérifier la compatibilité plus tard dans un useEffect
        console.log('📖 CheckID chargé depuis localStorage:', stored);
        if (urlParcoursId) {
          console.log('⚠️ ParcoursId dans URL détecté, validation de compatibilité nécessaire');
        }
        return stored;
      }
  ...
  // 🎯 NOUVEAU: Valider la compatibilité checkId/parcoursId au montage
  useEffect(() => {
    const validateCheckIdParcoursCompatibility = async () => {
      if (!currentCheckId) return;

      const urlParams = new URLSearchParams(window.location.search);
      const urlParcoursId = urlParams.get('parcours');

      // Si pas de parcoursId dans l'URL, pas de validation nécessaire
      if (!urlParcoursId) return;

      try {
        const session = await checkSessionManager.getCheckSession(currentCheckId);

        if (session && session.parcoursId !== urlParcoursId) {
          console.warn('⚠️ ActiveCheckIdContext: ParcoursId mismatch détecté!');
          console.warn('⚠️ Session parcoursId:', session.parcoursId);
          console.warn('⚠️ URL parcoursId:', urlParcoursId);
          console.warn('⚠️ Nettoyage du checkId incompatible');

          // Nettoyer le checkId incompatible
          setCurrentCheckId(null);
          localStorage.removeItem(STORAGE_KEY_ACTIVE);
        }
      } catch (error) {
        console.error('❌ Erreur validation compatibilité:', error);
      }
    };

    validateCheckIdParcoursCompatibility();
