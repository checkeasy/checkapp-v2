...
export const GlobalParcoursProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = parcoursManager.subscribe((parcours) => {
      console.log('📢 GlobalParcoursContext: Notification de changement:', {
  ...
  // 🎯 FIX: Trier les rooms par ordre CROISSANT (1, 2, 3...) pour garantir l'affichage correct
  // L'ordre 1 doit être la PREMIÈRE pièce affichée en haut du sélecteur
  const rooms = currentParcours
    ? Object.values(currentParcours.adaptedData.roomsData).sort((a, b) => a.ordre - b.ordre)
    : [];

  // 🎯 DEBUG: Afficher l'ordre final des rooms après tri
  if (rooms.length > 0) {
    console.log('📋 Ordre final des rooms après tri (CROISSANT):', rooms.map((r, i) => ({
      index: i,
      nom: r.nom,
      ordre: r.ordre,
      id: r.id,
      isPremiere: i === 0 ? '✅ PREMIÈRE' : ''
    })));
  }
