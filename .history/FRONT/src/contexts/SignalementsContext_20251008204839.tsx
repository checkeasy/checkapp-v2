...
export const SignalementsProvider = ({ children }: SignalementsProviderProps) => {
  ...
  const sendSignalementToBubble = async (signalement: Signalement, checkId: string) => {
    try {
      ...
      // Préparer le body de la requête (n'inclure base64 et imageurl que s'ils existent)
      const requestBody: {
        parcour: string;
        commentaire: string;
        pieceid: string;
      ...
      // Ajouter base64 uniquement si présent
      if (signalement.imgBase64) {
        requestBody.base64 = signalement.imgBase64;
      }

      // Ajouter imageurl uniquement si présent
      if (signalement.imgUrl) {
        requestBody.imageurl = signalement.imgUrl;
      }
      ...
      console.log('🚀 SignalementsContext: Envoi du signalement à Bubble:', {
        ...
        hasImageUrl: !!signalement.imgUrl,
        ...
        requestBodyBase64Length: requestBody.base64?.length || 0
      });
