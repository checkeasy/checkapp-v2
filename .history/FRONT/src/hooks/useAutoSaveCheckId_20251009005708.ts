...
export const useAutoSaveCheckId = () => {
  ...
  const savePhotoTaken = useCallback(async (
    ...
    try {
      // 🎯 FIX: Extraire le vrai etapeId depuis metadata si disponible
      // metadata.etapeId contient le vrai etapeId (sans pieceId)
      // taskId peut contenir "pieceId_etapeId"
      let realEtapeId = taskId;

      if (metadata?.etapeId && typeof metadata.etapeId === 'string') {
        // Utiliser metadata.etapeId en priorité (le plus fiable)
        realEtapeId = metadata.etapeId;
      } else if (taskId && taskId.includes('_')) {
        // Extraire depuis taskId si format "pieceId_etapeId"
        const parts = taskId.split('_');
        const extractedId = parts[parts.length - 1];
        if (/^\d+x\d+$/.test(extractedId)) {
          realEtapeId = extractedId;
        }
      }
      ...
      console.log('📝 Préparation données photo:', {
        ...
        taskId,
        pieceId,
        'metadata.etapeId': metadata?.etapeId,
      ...
      await interactionTracker.trackPhotoTaken({
        ...
        taskId,
        pieceId,
        etapeId: realEtapeId,  // ✅ Utilise le vrai etapeId extrait
        photoData,
        timestamp: new Date().toISOString(),
        validated: false,
        retakeCount: 0,
        metadata: {
          ...metadata,
          etapeId: realEtapeId,  // ✅ S'assurer que metadata.etapeId est correct
          checkId: currentCheckId,
          autoSaved: true,
          etapeId: realEtapeId
        }
