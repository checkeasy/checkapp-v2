      ...
      if (currentCheckId) {
        try {
          ...
          if (checkboxStates) {
            ...
            console.log('📥 useCheckoutFlowManager: États chargés depuis IndexedDB:', {
              totalCheckboxes: Object.keys(checkboxStates).length,
              completedTasks: completedTasksMap.size
            });
          }
        } catch (error) {
          console.error('❌ useCheckoutFlowManager: Erreur chargement IndexedDB:', error);
        }
      }
