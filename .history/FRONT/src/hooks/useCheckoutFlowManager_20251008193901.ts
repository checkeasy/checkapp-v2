    ...
    if (!currentPieceId || !currentPiece || !currentTask) {
      ...
      const findFirstAvailableTask = () => {
        for (const piece of pieces) {
          ...
          // Si toutes les tâches sont complétées, prendre la première tâche
          if (piece.tasks.length > 0) {
            return {
              pieceId: piece.id,
              taskIndex: 0,
              taskName: piece.tasks[0].nom || 'Tâche sans nom'
            };
          }
        }

        // Fallback: première pièce, première tâche
