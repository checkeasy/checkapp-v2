...
// Interface pour les données de propriété réelles
...
// Fonction pour générer les données de propriété à partir des informations du parcours
const generatePropertyDataFromParcours = (parcoursInfo: { name: string; type: string; logement: string; takePicture: string; } | null): PropertyData => {
  if (!parcoursInfo) {
    // Données par défaut si pas d'informations de parcours
    return {
      address: "Adresse non disponible",
      wifi: {
        network: "Réseau non configuré",
  ...
  return {
    address: parcoursInfo.logement || "Adresse non disponible",
    wifi: {
      network: "Réseau WiFi",
      password: "Contactez le propriétaire"
    },
    parking: "Informations de parking à confirmer",
    access: "Informations d'accès à confirmer",
    airbnbLink: "",
    checkIn: "15:00",
    checkOut: "11:00"
  };
...
const getNavigationConfigFromData = (
  ...
  // Fonction helper pour préserver le paramètre parcours ET checkid dans la navigation
  const navigateWithParcours = (path: string) => {
    const urlParams = new URLSearchParams(window.location.search);
    const checkId = urlParams.get('checkid');
    
    console.log('🔗 NavigateWithParcours DEBUG:', {
      path,
      currentParcoursId,
      checkId,
      currentUrl: window.location.href,
      isCheckIdActive,
      currentCheckIdFromContext: currentCheckId
    });
    
    if (currentParcoursId) {
      const separator = path.includes('?') ? '&' : '?';
      let fullPath = `${path}${separator}parcours=${currentParcoursId}`;
      
      // Ajouter checkid s'il existe
      if (checkId) {
        fullPath += `&checkid=${checkId}`;
      }
...
export default function CheckEasy() {
  ...
  // Génère les données de propriété réelles à partir des informations du parcours corrigées
  const propertyData = generatePropertyDataFromParcours(correctedParcoursInfo);
    ...
    <div className="bg-background px-4 py-3">
        <div className="max-w-md mx-auto">
          ...
          <DataSourceToggle
            ...
            loading={loading}
            error={error}
            parcoursInfo={correctedParcoursInfo}
          />
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <div className="text-xs text-gray-500 mt-1">
                ...
                {useRealData ? ' (données réelles)' : ' (données test)'}
              </div>
              {/* Debug Info Navigation */}
              {correctedParcoursInfo && (
                <div className="text-xs text-blue-600 mt-1 space-y-1">
                  <div>👤 Utilisateur: {user?.type || 'Non défini'}</div>
                  <div>📋 Type parcours: {correctedParcoursInfo.type}</div>
                  <div>📸 Mode photo: {correctedParcoursInfo.takePicture}</div>
                  <div>🎯 Scénario: {navigationConfig.flowDescription}</div>
                  {parcoursInfo?.takePicture !== correctedParcoursInfo.takePicture && (
                    <div className="text-xs text-orange-600">
                      🔧 CORRIGÉ: {parcoursInfo?.takePicture} → {correctedParcoursInfo.takePicture}
                    </div>
                  )}
                  <div className="flex gap-1">
                    ...
                    <button 
                      onClick={async () => {
                        ...
                        const urlParams = new URLSearchParams(window.location.search);
                        const parcoursId = urlParams.get('parcours');
                        console.log('URL parcours ID:', parcoursId);
                        
                        // DIAGNOSTIC SPÉCIAL pour les parcours problématiques
                        if (parcoursId === '1752234751041x918309083502084100' || parcoursId === '1756977102467x177831222533160960') {
                          console.log('Type attendu:', parcoursId === '1752234751041x918309083502084100' ? 'checkInAndCheckOut' : 'checkOutOnly');
                        }
                        ...
                        try {
                          ...
                          console.error('Erreur API directe:', e);
                        }
                        
                        // 3. Vérifier les données dans l'app
                        console.log('📱 Données dans l\'app:', {
                          original: parcoursInfo?.takePicture,
                          corrected: correctedParcoursInfo?.takePicture
                        });
                        
                        // 3.5. Vérifier les données du ParcoursManager
                        if (currentParcours) {
                          console.log('Raw data takePicture:', currentParcours.rawData?.takePicture);
                          console.log('Adapted data takePicture:', currentParcours.adaptedData?.parcoursInfo?.takePicture);
                        }
                        
                        // 4. Vérifier le cache localStorage
                        const cacheKeys = Object.keys(localStorage).filter(k => k.includes('parcours'));
                        cacheKeys.forEach(key => {
                          try {
                            const data = JSON.parse(localStorage.getItem(key) || '{}');
                            if (data.takePicture) {
                              console.log(`Cache ${key}:`, data.takePicture);
                            }
                          } catch (e) {
                    ...
                    <button 
                      onClick={async () => {
                        const targetId = '1756977102467x177831222533160960';
                        try {
                          // 1. Vider le cache localStorage
                          Object.keys(localStorage).forEach(key => {
                            if (key.includes('parcours') || key.includes('cache')) {
                              localStorage.removeItem(key);
                            }
                          });
                          
                          // 2. Vider le cache IndexedDB si possible
                          if ('indexedDB' in window) {
                            try {
                              const dbs = await indexedDB.databases();
                              for (const db of dbs) {
                                if (db.name && db.name.includes('parcours')) {
                          ...
                          // 3. Forcer le rechargement avec le bon URL
                          const newUrl = `${window.location.pathname}?parcours=${targetId}&_t=${Date.now()}`;
                          window.location.href = newUrl;
                          
                        } catch (error) {
                          console.error('❌ Erreur lors de la correction:', error);
                          // Fallback simple
                          const newUrl = `${window.location.pathname}?parcours=${targetId}`;
                          window.location.href = newUrl;
                        }
                      }}
                    ...
                    <button 
                      onClick={() => {
                        console.log('💥 SOLUTION RADICALE - Redirection immédiate');
                        
                        // Forcer la redirection avec timestamp pour éviter le cache
                        const targetId = '1756977102467x177831222533160960';
                        const timestamp = Date.now();
                        const newUrl = `${window.location.pathname}?parcours=${targetId}&_force=${timestamp}&_nocache=true`;
                    ...
                    <button 
                      onClick={async () => {
                        try {
                          const result = await debugService.sendDebugPayload();
                          
                          if (result.success) {
                            toast({
                              title: "Debug envoyé",
                              description: `Payload envoyé avec succès. ID: ${result.debugId}`,
                            });
                            } else {
                            toast({
                              title: "Erreur debug",
                            ...
                            console.error('❌ Erreur debug payload:', result.error);
                          }
                        } catch (error) {
                          toast({
                            title: "Erreur debug",
                            description: "Impossible d'envoyer le payload de debug",
                            variant: "destructive"
                          });
                          console.error('❌ Exception debug:', error);
                        }
                      }}
      ...
      <div className="max-w-md mx-auto px-4 space-y-2">
              ...
              <CardHeader className="py-3">
                <CardTitle className="text-base flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium">Debug - Envoyer le rapport complet</span>
                  </div>
                  <ChevronRight className="h-4 w-4 text-muted-foreground" />
                </CardTitle>
                <p className="text-xs text-muted-foreground mt-1">
                  Collecte et envoie toutes les données vers l'endpoint de debug pour analyse
                </p>
              </CardHeader>
            </Card>
          </DebugModal>
        ...
        {/* Bouton test webhooks */}
        {environment.IS_DEV && (
          <WebhookTestModal>
            <Card className="border border-purple-200 cursor-pointer hover:bg-purple-50 transition-colors">
              <CardHeader className="py-3">
                <CardTitle className="text-base flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-lg">🚀</span>
                    <span className="text-sm font-medium">Test Webhooks - Checkin & Checkout</span>
                  </div>
                  <ChevronRight className="h-4 w-4 text-muted-foreground" />
                </CardTitle>
                <p className="text-xs text-muted-foreground mt-1">
                  Teste l'envoi des données de checkin et checkout vers les webhooks Bubble
                </p>
              </CardHeader>
            </Card>
          </WebhookTestModal>
