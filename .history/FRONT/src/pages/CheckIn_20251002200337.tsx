...
export const CheckIn = ({
  ...
  useEffect(() => {
    const loadStateFromCheckId = async () => {
      ...
      try {
        ...
        // Restaurer la position dans le flow si disponible
        if (session.progress.currentPieceId && session.progress.currentTaskIndex !== undefined) {
          console.log('🎯 CheckIn: Restauration position:', {
            pieceId: session.progress.currentPieceId,
            taskIndex: session.progress.currentTaskIndex
          });

          jumpToPiece(session.progress.currentPieceId, session.progress.currentTaskIndex);
        }

        // TODO: Restaurer les photos et checkboxes (à implémenter comme dans CheckOut)
        console.log('ℹ️ CheckIn: Restauration photos/checkboxes à implémenter');
  ...
  }, [effectiveCheckId, isCheckIdActive, jumpToPiece]);
