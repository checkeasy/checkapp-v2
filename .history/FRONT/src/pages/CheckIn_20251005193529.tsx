  ...
  useEffect(() => {
    const loadStateFromCheckId = async () => {
      ...
      try {
        ...
        // 🎯 FIX CRITIQUE: Restaurer les photos capturées dans le state React
        if (savedPhotos) {
          console.log('📸 CheckIn: Restauration photos depuis IndexedDB...', savedPhotos);

          // Convertir l'objet photosTaken en Map pour capturedPhotosData
          const photosMap = new Map<string, CapturedPhoto[]>();

          Object.entries(savedPhotos).forEach(([taskId, photos]: [string, any]) => {
            if (Array.isArray(photos) && photos.length > 0) {
              photosMap.set(taskId, photos);
              console.log(`✅ ${photos.length} photos restaurées pour la tâche ${taskId}`);
            }
          });

          if (photosMap.size > 0) {
            setCapturedPhotosData(photosMap);
            console.log(`✅ Total: ${photosMap.size} tâches avec photos restaurées`);
          }
        }
