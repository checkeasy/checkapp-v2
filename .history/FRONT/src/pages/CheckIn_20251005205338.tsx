    ...
    const loadStateFromCheckId = async () => {
      ...
      try {
        ...
        if (savedPhotos) {
          ...
          // ✅ CORRECTION: Restaurer dans flowState.takenPhotos via addTakenPhotos
          // au lieu de capturedPhotosData (qui n'est pas utilisé par getTakenPhotos)
          Object.entries(savedPhotos).forEach(([taskId, photos]: [string, any]) => {
            if (Array.isArray(photos) && photos.length > 0) {
              // Convertir les photos sauvegardées en PhotoReference[]
              const photoReferences: PhotoReference[] = photos.map((photo: any) => ({
                tache_id: photo.taskId || taskId,
                etapeID: photo.etapeId || photo.photoId || '',
                url: photo.url || photo.dataUrl || '',
                expected_orientation: 'portrait' as const,
                overlay_enabled: false
              }));

              addTakenPhotos(taskId, photoReferences);
              console.log(`✅ ${photoReferences.length} photos restaurées pour la tâche ${taskId}`);
            }
          });

          console.log(`✅ Photos restaurées dans flowState.takenPhotos`);
        }
