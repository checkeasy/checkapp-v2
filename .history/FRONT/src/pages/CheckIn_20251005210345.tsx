  ...
  useEffect(() => {
    const loadStateFromCheckId = async () => {
      ...
      try {
        ...
        // 🎯 FIX CRITIQUE: Restaurer les photos capturées dans flowState.takenPhotos
        if (savedPhotos) {
          console.log('📸 CheckIn: Restauration photos depuis IndexedDB...', savedPhotos);

          // ✅ CORRECTION: Les photos sont indexées par photoId dans IndexedDB
          ...
          // Restaurer dans flowState.takenPhotos via addTakenPhotos
          photosByTaskId.forEach((photos, taskId) => {
            addTakenPhotos(taskId, photos);
            console.log(`✅ ${photos.length} photos restaurées pour la tâche ${taskId}`);
          });

          console.log(`✅ ${photosByTaskId.size} tâches avec photos restaurées dans flowState.takenPhotos`);
        }
