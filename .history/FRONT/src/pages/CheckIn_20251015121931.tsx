  ...
  const handleValidatePiece = async (taskId: string) => {
    ...
    setTimeout(async () => {
      if (flowState.isCompleted) {
        ...
        console.log('🔍 CheckIn: Conditions transition:', {
          takePicture: currentParcours?.rawData?.takePicture,
          shouldTransitionToCheckout,
          isCompleted: flowState.isCompleted
        });
        ...
        if (checkIdFromUrl) {
          try {
            ...
            if (session) {
              ...
              console.log('✅ CheckIn: État initial marqué comme complété dans la session');
            }
          } catch (error) {
            console.error('❌ CheckIn: Erreur sauvegarde état initial:', error);
          }
        }
        ...
        if (shouldTransitionToCheckout) {
          ...
          // Démarrer le flux checkout
          startCheckout();

          // Naviguer vers checkout
          navigateWithParams('/checkout');
        } else {
          // 🆕 REFACTORISÉ: Comportement original - naviguer vers checkin-home
