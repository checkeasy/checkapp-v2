...
export const CheckIn = ({
  ...
  // 🎯 FIX CRITIQUE: Utiliser allTasksCompleted au lieu de flowState.isCompleted
  useEffect(() => {
  ...
  const currentRoomSignalements = useMemo(() => {
    ...
    return [...userSignalements, ...apiSignalements];
  ...
  const handleValidatePiece = async (taskId: string) => {
    ...
    setTimeout(() => {
      if (flowState.isCompleted) {
        // 🎯 La transition est maintenant gérée par le useEffect (lignes 412-466)
        // qui détecte automatiquement flowState.isCompleted et effectue la bonne navigation
        console.log('✅ CheckIn: Flow complété, le useEffect va gérer la transition');
      } else {
        nextStep();
      }
    }, 1000);
  ...
  const handlePhotosCaptured = async (capturedPhotos: CapturedPhoto[]) => {
    ...
    if (syncedCurrentTask) {
      ...
      setTimeout(() => {
        if (flowState.isCompleted) {
          // 🎯 La transition est maintenant gérée par le useEffect (lignes 412-466)
          console.log('✅ CheckIn: Flow complété après photos, le useEffect va gérer la transition');
        } else {
          nextStep();
        }
      }, 1000);
      ...
      <div className="flex-1 px-3 py-4 space-y-4 pb-24">
        ...
        <div className="space-y-4">
          {syncedCurrentTask && (() => {
            ...
            return ['photo_multiple', 'photo_optional', 'photo_required', 'photo_validation', 'reference_photos'].includes(syncedCurrentTask.type) ? (
              <div className="space-y-4">
                <RoomTaskCard
