...
export const CheckinHome = () => {
  ...
  useEffect(() => {
    ...
    // 2. Le parcours est terminé (checkin ou checkout complété)
    const shouldPoll =
      checkSession?.status === 'terminated' &&
      checkSession?.rapportID &&
      (checkinFlowState?.isCompleted || checkoutFlowState?.isCompleted || appFlowState?.checkinCompleted || appFlowState?.checkoutCompleted);

    if (!shouldPoll || isRapportReady) {
      return; // Ne pas démarrer le polling si conditions non remplies ou rapport déjà prêt
    }

    console.log('📋 Démarrage du polling pour vérifier le statut du rapport');
    ...
    const checkRapportStatus = async () => {
      ...
      if (ready) {
        console.log('✅ Rapport IA terminé et prêt à être consulté');
    ...
    // Cleanup
    return () => {
      console.log('📋 Arrêt du polling du statut du rapport');
      clearInterval(intervalId);
    };
  }, [checkSession?.status, checkSession?.rapportID, currentCheckId, isRapportReady, checkinFlowState?.isCompleted, checkoutFlowState?.isCompleted, appFlowState?.checkinCompleted, appFlowState?.checkoutCompleted]);
