...
export const CheckinHome = () => {
  ...
  useEffect(() => {
    ...
    const checkRapportStatus = async () => {
      ...
      console.log('🔄 CheckinHome: Vérification du statut du rapport pour checkId:', currentCheckId);
      const ready = await rapportStatusService.isRapportReady(currentCheckId);
      console.log('📊 CheckinHome: Résultat de la vérification:', { ready });
    ...
    if ((isAgent || isGestionnaire) && parcoursInfo.takePicture === 'checkOutOnly') {
      return {
        ...
        action: async () => {
          ...
          // 🎯 NOUVEAU: Réinitialiser la position à la première pièce ET mettre à jour flowType
          if (currentCheckId) {
            try {
              const session = await checkSessionManager.getCheckSession(currentCheckId);
              if (session && rooms.length > 0) {
                ...
                console.log('✅ CheckinHome: flowType mis à jour vers "checkout" dans la session');
              }
            } catch (error) {
