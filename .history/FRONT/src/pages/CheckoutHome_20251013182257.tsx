...
export const CheckoutHome = () => {
  ...
  useEffect(() => {
    ...
    // 2. Le parcours est terminé (checkout complété)
    const shouldPoll =
  ...
  }, [checkSession?.status, checkSession?.rapportID, currentCheckId, isRapportReady, flowState?.isCompleted]);
  ...
  const getCheckoutButtonText = () => {
    ...
    if (isSessionTerminated && checkSession?.rapportID) {
      return isRapportReady ? "📋 Voir mon rapport" : "📋 Rapport en cours...";
    }
  ...
  const getCheckoutButtonIcon = () => {
    if (isSessionTerminated && checkSession?.rapportID) {
      return isRapportReady ? <FileText className="h-4 w-4" /> : <Loader2 className="h-4 w-4 animate-spin" />;
    }
    return undefined;
  };
    ...
    <div className="min-h-screen bg-background">
      ...
      <CTASection
        primaryAction={{
          ...
          icon: getCheckoutButtonIcon(),
          disabled: (flowState.isCompleted && !isSessionTerminated) || (isSessionTerminated && checkSession?.rapportID && !isRapportReady)
        }}
        bottomText={{
