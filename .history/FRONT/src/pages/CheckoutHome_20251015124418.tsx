...
export const CheckoutHome = () => {
  ...
  const hasCheckoutProgress = () => {
    // 🎯 CORRECTION CRITIQUE: Vérifier la session réelle, pas le flowState
    // Le flowState est automatiquement initialisé dès que les rooms sont chargées,
    // donc flowState.currentStep.pieceId sera toujours défini même si l'utilisateur
    // n'a jamais cliqué sur le bouton checkout
    ...
    // Vérifier si des tâches ont été complétées dans le flowState
    const hasCompletedTasks = Object.keys(flowState.completedTasks).length > 0;

    // Vérifier si la session a un statut autre que 'active' initial
    const hasProgressed = checkSession.status !== 'active' ||
                         checkSession.lastActiveAt !== checkSession.createdAt;

    console.log('🔍 CheckoutHome hasCheckoutProgress:', {
      hasInteractions,
      hasCompletedTasks,
      hasProgressed,
      result: hasInteractions || hasCompletedTasks
    });
