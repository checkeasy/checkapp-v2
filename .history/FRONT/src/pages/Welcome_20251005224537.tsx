...
const Welcome = () => {
  ...
  // 🎯 FIX: Ajout du logementId pour filtrer par logement
  const checkForExistingSessions = async (userId: string, parcoursId?: string) => {
    ...
    try {
      // 🎯 FIX: Extraire le logementId depuis le parcours ou les infos globales
      let logementId: string | undefined;

      if (globalParcoursInfo?.logement) {
        logementId = globalParcoursInfo.logement;
      } else if (parcoursId) {
        // Extraire depuis parcoursId si format "logement_XXX_parcours_YYY"
        const match = parcoursId.match(/logement_[^_]+/);
        logementId = match ? match[0] : parcoursId;
      }

      console.log('🏠 Welcome: LogementId extrait:', logementId);

      // 1. Récupérer la liste complète des sessions utilisateur pour CE logement
      const userSessions = await checkSessionManager.getUserSessionsList(userId, logementId);

      if (!userSessions.hasAnySessions) {
        console.log('❌ Welcome: Aucune session trouvée pour cet utilisateur dans ce logement');
        return false;
      }

      console.log('📊 Welcome: Sessions trouvées pour ce logement:', {
        total: userSessions.totalCount,
        active: userSessions.activeSessions.length,
        completed: userSessions.completedSessions.length,
        logements: Object.keys(userSessions.sessionsByLogement || {}).length
      });

      // 🎯 NOUVEAU: Toujours afficher la liste complète des sessions de CE logement
