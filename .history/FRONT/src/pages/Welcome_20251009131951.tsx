...
const Welcome = () => {
  ...
  useEffect(() => {
    const restoreSessionFromUrl = async () => {
      ...
      try {
        ...
        // 🎯 FIX CRITIQUE: Vérifier que le parcoursId correspond
        if (urlParcoursId && session.parcoursId !== urlParcoursId) {
          console.error('❌ Welcome: ParcoursId mismatch!');
          console.error('❌ Session parcoursId:', session.parcoursId);
          console.error('❌ URL parcoursId:', urlParcoursId);
          toast({
            title: "Parcours incompatible",
            description: "Ce checkID appartient à un autre parcours. Veuillez recommencer.",
            variant: "destructive"
          });
        ...
        // 🎯 IMPORTANT: Vérifier d'abord si RouteRestoration a déjà fait son travail
        const savedPath = session.progress?.navigation?.lastPath || localStorage.getItem('checkeasy_last_path');

        // 🎯 FIX CRITIQUE: Utiliser le chemin actuel comme source de vérité
        // Si RouteRestoration nous a déjà mis sur /checkout, rester sur /checkout
        // Ne PAS se fier au flowType de la session qui peut être obsolète
        const currentPath = location.pathname;
        const isOnCheckout = currentPath === '/checkout';
        const isOnCheckin = currentPath === '/checkin';

        // Déterminer le flowType réel basé sur le chemin actuel
        const actualFlowType = isOnCheckout ? 'checkout' : isOnCheckin ? 'checkin' : session.flowType;

        console.log('🔍 Welcome: Données de restauration:', {
          savedPath,
          ...
          isOnCheckout,
          isOnCheckin,
          hasNavigationData: !!session.progress?.navigation,
          navigationData: session.progress?.navigation
        });

        // 🎯 FIX CRITIQUE: Ne PAS rediriger si on est déjà sur la bonne page
        // RouteRestoration a déjà fait son travail
        if (location.pathname !== '/welcome') {
          console.log('✅ Welcome: Déjà sur la bonne page, skip redirection:', location.pathname);

          // 🎯 NOUVEAU: Mettre à jour le flowType de la session si nécessaire
          if (actualFlowType !== session.flowType) {
            console.log('🔄 Welcome: Mise à jour flowType de la session:', {
              ancien: session.flowType,
              nouveau: actualFlowType
            });

            // Mettre à jour la session avec le bon flowType
            await checkSessionManager.updateCheckSession(urlCheckId, {
        ...
        // Déterminer la route correcte selon le flowType et lastPath
        let targetPath = '/';

        if (savedPath) {
          // Si on a un lastPath sauvegardé, l'utiliser en priorité
          targetPath = savedPath;
          console.log('✅ Welcome: Utilisation de lastPath sauvegardé:', savedPath);
        } else {
          // Sinon, fallback sur le actualFlowType (pas session.flowType)
          if (actualFlowType === 'checkout') {
            targetPath = '/checkout';
          } else if (actualFlowType === 'checkin') {
            targetPath = '/checkin';
          }
          console.log('⚠️ Welcome: lastPath manquant, fallback sur actualFlowType:', actualFlowType, '→', targetPath);
        }
  ...
  const createNewSessionAndLogin = async () => {
    ...
    // 🎯 FIX CRITIQUE: Récupérer le parcoursId depuis l'URL si le state est null
    // Cela évite les problèmes de timing où le state n'est pas encore mis à jour
    const effectiveParcoursId = parcoursId || getUrlParameter('parcours');

    console.log('🔍 Welcome: ParcoursId effectif:', {
      fromState: parcoursId,
      fromUrl: getUrlParameter('parcours'),
    ...
    if (!effectiveParcoursId || !globalParcoursInfo) {
      ...
      const params = new URLSearchParams();
      if (effectiveParcoursId) params.set('parcours', effectiveParcoursId);
    ...
    try {
      ...
      // 🆕 FIX: Naviguer avec les paramètres parcours ET checkid
      const params = new URLSearchParams();
      if (effectiveParcoursId) params.set('parcours', effectiveParcoursId); // 🎯 FIX: Utiliser effectiveParcoursId
