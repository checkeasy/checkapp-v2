    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = (event) => {
        ...
        if (!db.objectStoreNames.contains(this.storeName)) {
          ...
          store.createIndex('logementId', 'logementId', { unique: false }); // 🎯 FIX: Index pour logementId
          store.createIndex('userLogement', ['userId', 'logementId'], { unique: false }); // 🎯 FIX: Index composite
          store.createIndex('status', 'status', { unique: false });
          store.createIndex('createdAt', 'createdAt', { unique: false });
          console.log('✅ Store checkSessions créé avec index composite userLogement');
        }
      };
    });
  }
