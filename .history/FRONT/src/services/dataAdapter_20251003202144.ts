...
export class DataAdapter {
  ...
  private static adaptPieceToRoom(realPiece: RealPiece, ordre: number): Room | null {
    ...
    const roomInfo = realPiece.travelerNote || realPiece.infoEntrance || `Informations pour ${this.cleanRoomName(realPiece.nom)}`;
    const cleaningInfo = realPiece.cleanerNote || `Instructions de nettoyage pour ${this.cleanRoomName(realPiece.nom)}`;
