...
export class DataAdapter {
  ...
  private static createTaskFromEtape(
    ...
    // 🎯 NOUVEAU: Vérifier si la tâche nécessite une photo
    // Une photo est requise si:
    // 1. todoImage existe (photo de référence fournie)
    // 2. OU todoParam === "photoRequired" (photo requise sans référence)
    // 3. IMPORTANT: Si todoParam est vide (""), la photo n'est PAS requise
    const requiresPhoto = !!etape.todoImage || (etape.todoParam && etape.todoParam.trim() !== '' && etape.todoParam === 'photoRequired');
    ...
    // 🎯 NOUVEAU: Créer une photo_reference même si todoImage n'existe pas
    // Cela permet au PhotoCaptureModal de fonctionner correctement
    let photoReference = undefined;
    if (etape.todoImage) {
      // Cas normal: photo de référence fournie
      photoReference = {
      ...
      // Cas spécial: photo requise sans référence
