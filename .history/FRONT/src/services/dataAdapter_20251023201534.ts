...
export class DataAdapter {
  ...
  static generateTasksFromRealData(
    ...
    // 2. ENSUITE : Ajouter les tâches de vérification pour TOUS les modes (checkin ET checkout)
  ...
  private static createTaskFromEtape(
    ...
    // 🎯 FIX: Utiliser todoParam pour les instructions détaillées au lieu de todoOrder
    // todoOrder contient juste l'ordre (ex: "1", "2"), todoParam contient les vraies instructions
    const instructions = etape.todoParam || etape.todoOrder;

    // 🎯 CRITICAL FIX: Vérifier si la tâche nécessite une photo
    // Une photo est requise UNIQUEMENT si:
    ...
    //
    // IMPORTANT: Si todoParam est vide (""), la photo n'est JAMAIS requise,
    // même si todoImage existe. Un todoParam vide signifie "checkbox simple sans photo".
    ...
    // 🎯 NOUVEAU: Si photoRequired mais pas d'instructions, utiliser un message par défaut
    let taskDescription = instructions?.trim();
    if (etape.todoParam === 'photoRequired' && !etape.todoImage) {
      // Ne pas afficher "photoRequired" comme description
      taskDescription = undefined;
    }

    // 🎯 CRITICAL FIX: Créer une photo_reference UNIQUEMENT si la photo est requise
    // Ne pas créer de photo_reference si todoParam est vide, même si todoImage existe
    let photoReference = undefined;
    if (requiresPhoto) {
      if (etape.todoImage && hasPhotoRequirement) {
        // Cas normal: photo de référence fournie ET todoParam non vide
        photoReference = {
      ...
      } else if (etape.todoParam === 'photoRequired') {
        // Cas spécial: photo requise sans référence (todoParam = "photoRequired")
        // Créer une photo_reference avec une image placeholder
        photoReference = {
          tache_id: etape.etapeID,
          etapeID: etape.etapeID,
          url: '/placeholder-photo-required.jpg', // Image placeholder pour indiquer qu'une photo est requise
          expected_orientation: 'paysage' as const,
          overlay_enabled: false, // Pas d'overlay car pas de vraie référence
          isTodo: true,
          todoTitle: title.trim()
        };
      }
