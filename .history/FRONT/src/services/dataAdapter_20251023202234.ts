...
export class DataAdapter {
  ...
  private static createTaskFromEtape(
    ...
    // 🎯 CRITICAL FIX: Créer une photo_reference pour afficher l'image de référence
    // Deux cas:
    // 1. todoImage existe → Afficher comme référence visuelle (même si todoParam est vide)
    // 2. todoParam === "photoRequired" → Photo obligatoire sans référence (placeholder)
    let photoReference = undefined;
    if (etape.todoImage) {
      // Cas 1: Photo de référence fournie (affichée même si todoParam est vide)
      ...
      // Cas 2: Photo obligatoire sans référence (todoParam = "photoRequired")
