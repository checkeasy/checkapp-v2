  ...
  private async _loadParcoursDataInternal(
    ...
    try {
      ...
      if (cacheValid && cachedData) {
        ...
        // 🆕 FIX: Vérifier que cachedData n'est pas null avant de l'utiliser
        if (!cachedData) {
          console.warn(`⚠️ [DataLoadingOrchestrator] Cache invalide (données null), rechargement depuis l'API...`);
        } else {
