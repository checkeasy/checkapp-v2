...
class DebugService {
  ...
  async generateEnrichedAPIFormat(): Promise<EnrichedAPIPayload | null> {
    try {
      ...
      const enrichedPayload: EnrichedAPIPayload = {
        ...
        stats: this.calculateStats(donneesUtilisateur, baseData),
   ...
   */
  private calculateStats(donneesUtilisateur: any, baseData: DebugPayload): EnrichedAPIPayload['stats'] {
  ...
  async sendUnifiedWebhook(customUrl?: string): Promise<DebugResult> {
    try {
      ...
      // Générer le format enrichi complet avec TOUT
      const enrichedPayload = await this.generateEnrichedAPIFormat();

      if (!enrichedPayload) {
        console.error('❌ Impossible de générer les données complètes');
        return {
          success: false,
          timestamp: new Date().toISOString(),
          error: 'Impossible de générer les données complètes'
        };
      }
      ...
      console.log('🎯 DONNÉES COMPLÈTES collectées:', {
        ...
        totalPieces: enrichedPayload.stats.totalPieces,
        photosCheckin: enrichedPayload.stats.photosCheckin,
        photosCheckout: enrichedPayload.stats.photosCheckout,
        hasCheckin: !!enrichedPayload.checkin,
        hasCheckout: !!enrichedPayload.checkout,
        checkinPieces: enrichedPayload.checkin?.pieceResults.length || 0,
        checkoutPieces: enrichedPayload.checkout?.pieceResults.length || 0,
        signalements: enrichedPayload.stats.signalementsTotal
      });

      // Envoyer au webhook unifié
      const result = await this.sendToUnifiedWebhook(enrichedPayload, customUrl);
  ...
  private async sendToUnifiedWebhook(payload: EnrichedAPIPayload, customUrl?: string): Promise<DebugResult> {
    // URL de l'endpoint unifié (par défaut ou personnalisée)
    const UNIFIED_WEBHOOK_URL = customUrl || 'https://checkeasy-57905.bubbleapps.io/version-test/api/1.1/wf/checkendpoint/initialize';
