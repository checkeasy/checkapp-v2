...
class DebugService {
  ...
  async sendUnifiedWebhook(customUrl?: string): Promise<DebugResult> {
    try {
      ...
      const webhookPayload = await generateUnifiedWebhookData(session, currentCheckId);
      ...
      const result = await this.sendToUnifiedWebhookEndpoint(webhookPayload, customUrl);
