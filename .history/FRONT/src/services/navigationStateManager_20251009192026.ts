...
class NavigationStateManager {
  ...
  isRouteAllowed(currentPath: string, session: CheckSession): boolean {
    ...
    if (session.status === 'active') {
      ...
      if (session.flowType === 'checkout') {
        // 🆕 FIX: Ajouter /checkin-home pour les parcours "Ménage avec état initial"
        // Dans ce flux, l'utilisateur peut être sur /checkin-home avant de commencer le ménage
        const allowedRoutes = ['/checkout', '/checkout-home', '/checkin-home', '/etat-initial', '/exit-questions'];
        return allowedRoutes.includes(currentPath);
      }
    }
