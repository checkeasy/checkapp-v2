  ...
  getCorrectRouteForSession(session: CheckSession): string {
    ...
    if (session.status === 'active') {
      ...
      if (session.flowType === 'checkout') {
        ...
        // Si etatInitialCompleted est false ou undefined, rediriger vers /etat-initial
        if (!session.progress.etatInitialCompleted) {
          // Vérifier si le parcours nécessite un état initial
          // Pour l'instant, on suppose que si etatInitialCompleted existe (même false), c'est un parcours avec état initial
          // TODO: Ajouter une vérification du type de parcours dans parcoursInfo
          return '/etat-initial';
        }
