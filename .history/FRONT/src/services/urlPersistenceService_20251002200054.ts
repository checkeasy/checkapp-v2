...
class UrlPersistenceService {
  ...
  restoreUrlParamsIfMissing(): boolean {
    try {
      ...
      if (!currentCheckid && saved.checkid) {
        newParams.set('checkid', saved.checkid);
        restored = true;
        console.log('🔄 Restored checkid param:', saved.checkid);
      }
  ...
  getCurrentState(): {
    ...
    return {
      ...
      hasCheckidInStorage: !!saved?.checkid,
      canRestore: !!(saved?.parcours || saved?.checkid)
    };
  }
