...
class UrlPersistenceService {
  ...
  restoreUrlParamsIfMissing(): boolean {
    try {
      ...
      // 🎯 CRITICAL FIX: Only restore checkid if URL ALREADY has one
      // This prevents auto-restoring checkid when user wants a new session
      if (currentCheckid && !currentCheckid) {
