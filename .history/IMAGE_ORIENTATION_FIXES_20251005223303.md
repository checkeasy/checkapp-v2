...
**Location**: `FRONT/src/hooks/usePhotoCapture.ts` (lines 48-52)

**Problem**: 
- The video preview has `transform: scaleX(-1)` applied in CSS for better UX (users see themselves normally)
- On iOS devices, an ADDITIONAL `ctx.scale(-1, 1)` transform was being applied during capture
- This caused captured photos to be mirrored, which is incorrect behavior
- Standard camera behavior: preview is mirrored, but final photo should NOT be mirrored

**Fix Applied**:
- Removed the iOS-specific mirror transform during capture
- Now all platforms capture the true un-mirrored image
- Preview remains mirrored (good UX), but captured photo is un-mirrored (correct)
...
### 2. `FRONT/src/hooks/usePhotoCapture.ts`
**Changes**:
