...
**Root Cause:** The `checkAutoAdvancement` function was not being called after completing tasks, so `flowState.isCompleted` was never set to `true`.

**Solution:** Added calls to `checkAutoAdvancement(actualPieces)` after:
