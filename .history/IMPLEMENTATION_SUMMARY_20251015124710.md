...
**Issue:** The "Continuer mon check de sortie" button always showed "Continuer" even when checkout hadn't started yet, because `hasCheckoutProgress()` was checking the `flowState` which is automatically initialized when rooms are loaded.

**Root Cause:** The `flowState` from CheckoutFlowContext is automatically initialized with `currentStep.pieceId` as soon as rooms are loaded, so the condition `flowState.currentStep.pieceId !== ''` was always true.

**Solution:**
1. Changed the button text from "Finaliser mon check de sortie" to "Commencer mon check de sortie" when checkout hasn't started
2. Fixed `hasCheckoutProgress()` to check ONLY the actual session data, NOT the `flowState`:
   - Checks ONLY if there are real user interactions in the session (photos taken, checkboxes checked, buttons clicked)
   - Does NOT check `flowState.completedTasks` because it's automatically filled by the context
   - Returns `true` only if there's actual user interaction in checkout, not just context initialization
   - This ensures the button text only changes when the user actually clicks the button and starts interacting
...
### 4. CheckoutHome.tsx - Button Label and Room Selection Fixes

**File Modified:** `FRONT/src/pages/CheckoutHome.tsx`

#### Fix 1: Button Label (line 331)
**Changes:**
- Changed button text from "Finaliser mon check de sortie" to "Commencer mon check de sortie" when checkout hasn't started yet
- The button now correctly shows:
  - "Commencer mon check de sortie" when `hasCheckoutProgress()` returns false
  - "Continuer mon check de sortie" when `hasCheckoutProgress()` returns true
...
#### Fix 1b: hasCheckoutProgress() Function (lines 308-341)
**Changes:**
- Completely rewrote `hasCheckoutProgress()` to check actual session data instead of `flowState`
- Now checks for real user interactions in the session:
  - Photos taken in checkout
  - Checkboxes checked in checkout
  - Buttons clicked in checkout
  - Tasks completed in flowState
- Returns `false` if the session doesn't exist or has no interactions
...
const hasCheckoutProgress = () => {
  ...
  // Vérifier s'il y a eu des interactions réelles dans le checkout
  const hasInteractions =
    (checkSession.progress?.interactions?.photosTaken &&
     Object.keys(checkSession.progress.interactions.photosTaken).length > 0) ||
    (checkSession.progress?.interactions?.checkboxStates &&
     Object.keys(checkSession.progress.interactions.checkboxStates).length > 0) ||
    (checkSession.progress?.interactions?.buttonClicks &&
     Object.keys(checkSession.progress.interactions.buttonClicks).length > 0);

  const hasCompletedTasks = Object.keys(flowState.completedTasks).length > 0;

  return hasInteractions || hasCompletedTasks;
};
```

#### Fix 2: Room Selection on Button Click (lines 256-296)
**Changes:**
- Modified `handleStartCheckout` to be async
- Added logic to reset checkout position to the first room before navigation
- Sorts rooms by `ordre` field (ascending)
- Saves first room ID and taskIndex 0 to session
