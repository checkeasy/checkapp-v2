...
export const CleaningInstructionsModal = ({ children, flowType = 'checkout' }: CleaningInstructionsModalProps) => {
  ...
  // 🐛 DEBUG: Afficher les données des pièces
  console.log('🧹 CleaningInstructionsModal: Données des pièces:', {
    roomsCount: rooms.length,
    flowType,
    rooms: rooms.map(r => ({
      id: r.id,
      nom: r.nom,
      hasCleaningInfo: !!r.cleaningInfo,
      cleaningInfo: r.cleaningInfo?.substring(0, 50) + '...',
      hasTravelerNote: !!r.travelerNote,
      hasCleanerNote: !!r.cleanerNote
    }))
  });
  ...
  const getCleaningTasks = (room: any) => {
    if (!room.tasks) {
      console.log(`⚠️ CleaningInstructionsModal: Pièce "${room.nom}" n'a pas de tâches`);
      return [];
    }

    console.log(`🔍 CleaningInstructionsModal: Analyse des tâches pour "${room.nom}":`, {
      totalTasks: room.tasks.length,
      tasks: room.tasks.map((t: any) => ({
        id: t.id,
        label: t.label,
        type: t.type,
        isTodo: t.isTodo
    ...
    const filteredTasks = room.tasks.filter((task: any) => {
      // Inclure uniquement les tâches de ménage (isTodo=true)
      if (task.isTodo) {
        console.log(`✅ Tâche incluse (isTodo=true): ${task.label}`);
        return true;
      }

      // Exclure les tâches de photos de référence
      if (task.type === 'photo_multiple' || task.type === 'photo_optional') {
        console.log(`❌ Tâche exclue (photo_multiple/optional): ${task.label}`);
        return false;
      }
      ...
      if (excludeKeywords.some(keyword => taskLabel.includes(keyword))) {
        console.log(`❌ Tâche exclue (mot-clé): ${task.label}`);
        return false;
      }

      console.log(`❌ Tâche exclue (défaut): ${task.label}`);
      return false;
    });

    console.log(`📊 CleaningInstructionsModal: Tâches filtrées pour "${room.nom}": ${filteredTasks.length}/${room.tasks.length}`);
                ...
                <AccordionItem key={room.id} value={room.id} className="border border-border/50 rounded-xl overflow-hidden bg-card">
                  <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-muted/50 transition-colors">
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      <span className="font-semibold capitalize truncate">{room.nom}</span>
