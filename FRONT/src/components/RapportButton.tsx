...
interface RapportButtonProps {
  ...
  onClick: () => void;
  className?: string;
  progress?: number; // 🎯 NOUVEAU: Pourcentage de progression (0-100)
}

export const RapportButton = ({ isReady, onClick, className, progress = 0 }: RapportButtonProps) => {
  return (
    <button
      onClick={onClick}
      disabled={!isReady}
      className={cn(
        // Base styles
        "group relative w-full h-10 px-4 rounded-lg font-medium text-sm",
    ...
    >
        ...
        {isReady ? (
          <>
            <div className="relative">
              <FileText className="h-4 w-4 transition-all duration-300 group-hover:scale-110 group-hover:rotate-3" />
            ...
            <span className="font-semibold tracking-wide">Voir mon rapport IA</span>
            {/* Flèche animée */}
            <svg
      ...
      {!isReady && (
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-primary/20 overflow-hidden rounded-b-xl">
          <div
            className="h-full bg-gradient-to-r from-primary/40 via-primary/60 to-primary/40 transition-all duration-500 ease-out"
      ...
      {isReady && (
        <div className="absolute inset-0 rounded-xl opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-purple-500 via-pink-500 to-purple-500 opacity-30 blur-lg animate-pulse-slow" />
          <div className="absolute inset-0 rounded-xl border border-primary opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
        </div>
      )}
    </button>
  );
