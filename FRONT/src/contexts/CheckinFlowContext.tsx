...
export const CheckinFlowProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const jumpToPiece = (pieceId: string, taskIndex: number = 0) => {
    setFlowState(prev => {
      // 🆕 FIX: Chercher d'abord une étape exacte, sinon créer une étape temporaire
      let targetStep = prev.flowSequence.find(step =>
        step.pieceId === pieceId && step.taskIndex === taskIndex
      );

      // Si l'étape exacte n'existe pas, créer une étape temporaire pour permettre la navigation
      if (!targetStep) {
        const baseStep = prev.flowSequence.find(step => step.pieceId === pieceId);
        if (baseStep) {
          targetStep = {
            ...baseStep,
            taskIndex: taskIndex
          };
          console.log('🔄 CheckinFlow: Création étape temporaire pour navigation:', { pieceId, taskIndex });
        }
      }
      ...
      console.warn('⚠️ CheckinFlow: Impossible de trouver la pièce:', pieceId);
      return prev;
    });
  };
  ...
  const checkAutoAdvancement = async (pieces: any[]) => {
    // 🆕 Pour CheckIn: Vérifier si toutes les PIÈCES sont validées (pas les tâches)
    // Une pièce est validée si:
    // 1. Son état dans pieceStates est 'validated'
    // 2. OU toutes ses tâches sont complétées (ancien comportement)

    try {
      if (!currentCheckId) {
        console.warn('⚠️ CheckinFlow: Pas de CheckID actif pour vérifier l\'avancement');
        return;
      }

      const session = await checkSessionManager.getCheckSession(currentCheckId);
      const pieceStates = session?.progress?.interactions?.pieceStates || {};

      // Vérifier si toutes les pièces sont validées
      const allPiecesValidated = pieces.every(piece => {
        const pieceState = pieceStates[piece.id];
        // Une pièce est validée si son état est 'validated' ou 'completed'
        return pieceState?.status === 'validated' || pieceState?.status === 'completed';
      });

      console.log('🔍 CheckinFlow checkAutoAdvancement:', {
        allPiecesValidated,
        piecesCount: pieces.length,
        validatedPiecesCount: pieces.filter(p => {
          const state = pieceStates[p.id];
          return state?.status === 'validated' || state?.status === 'completed';
        }).length,
        pieceStatesKeys: Object.keys(pieceStates)
      });

      if (allPiecesValidated) {
        console.log('✅ CheckinFlow: Toutes les pièces validées, marquage isCompleted = true');
        setFlowState(prev => ({ ...prev, isCompleted: true }));
      }
    } catch (error) {
      console.error('❌ CheckinFlow: Erreur vérification avancement:', error);
    }
  };
