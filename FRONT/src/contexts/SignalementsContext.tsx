...
export const SignalementsProvider = ({ children }: SignalementsProviderProps) => {
  ...
  const sendSignalementToBubble = async (signalement: Signalement, checkId: string) => {
    try {
      ...
      const requestBody: {
        ...
        reportid: checkId, // ID du rapport en cours
      };
    ...
    try {
      ...
      const loadedSignalements: Signalement[] = Object.entries(sessionSignalements).map(([key, sig]: [string, any]) => {
        ...
        return {
          ...
          parcoursId: sig.metadata?.parcoursId || sig.parcoursId, // ✅ NOUVEAU - Charger parcoursId
          checkId: sig.metadata?.checkId || currentCheckId, // ✅ NOUVEAU - Charger checkId depuis métadonnées ou utiliser le checkId actuel
          rapportRef: sig.metadata?.rapportRef || sig.rapportRef, // ✅ NOUVEAU - Charger rapportRef pour dédupliquer
        };
      });
  ...
  const addSignalement = async (signalementData: Omit<Signalement, 'id' | 'created_at' | 'updated_at'>) => {
    ...
    console.log('🚨 SignalementsContext: Création signalement avec parcoursId:', {
    ...
    const newSignalement: Signalement = {
      ...
      created_at: now,
      updated_at: now,
      parcoursId: currentParcours?.id, // ✅ NOUVEAU - Ajouter le parcoursId
      checkId: currentCheckId || undefined, // ✅ NOUVEAU - Ajouter le checkId
    };
      ...
      try {
        await interactionTracker.trackSignalement({
          ...
          metadata: {
            ...
            checkId: currentCheckId,            // ✅ NOUVEAU: Sauvegarder checkId
            parcoursId: currentParcours?.id     // ✅ NOUVEAU: Sauvegarder parcoursId
          }
        });
