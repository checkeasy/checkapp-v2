...
const getNavigationConfigFromData = (
  ...
  const navigateWithParcours = (path: string) => {
    ...
    const effectiveParcoursId = currentParcoursId || parcoursIdFromUrl;
    const effectiveCheckId = currentCheckId || checkIdFromUrl;
    ...
    if (!effectiveParcoursId) {
      console.error('❌ ERREUR CRITIQUE: Aucun parcoursId disponible pour la navigation !');
      navigate(path);
      return;
    }

    // 🆕 Utiliser navigationStateManager pour construire l'URL
    const fullPath = effectiveCheckId
      ? navigationStateManager.buildUrl(path, effectiveParcoursId, effectiveCheckId)
      : `${path}?parcours=${effectiveParcoursId}`;

    console.log('🚀 CheckEasy: Navigation vers:', fullPath);
    navigate(fullPath);
  };
  ...
  if (userType === 'CLIENT' && isCheckInAndOut) {
    return {
      buttonText: hasProgress ? "Continuer mon état des lieux d'entrée" : "Commencer mon état des lieux d'entrée",
      navigationAction: () => navigateWithParcours('/checkin'),
      shouldShowInitialStateScreen: false,
...
export default function CheckEasy() {
  ...
  const { isLoaded, apiSignalements } = useParcoursData();
  ...
  const { parcours: parcoursUnified, loading: parcoursUnifiedLoading } = useParcoursDataUnified(parcoursIdFromUrl, undefined);
  ...
  useEffect(() => {
    if (parcoursIdFromUrl && !isLoaded) {
      // Charger le parcours dans le contexte global pour compatibilité
      loadParcours(parcoursIdFromUrl).catch(error => {
        console.error('❌ CheckEasy: Erreur chargement parcours:', error);
      });
    }
  }, [parcoursIdFromUrl, isLoaded, loadParcours]);

  // 🆕 REFACTORISÉ: Synchronisation URL simplifiée
  useEffect(() => {
    if (parcoursInfo && currentParcours && !parcoursIdFromUrl) {
      // Si le parcours est chargé mais pas dans l'URL, l'ajouter
      const effectiveCheckId = currentCheckId || checkIdFromUrl;
      const correctUrl = effectiveCheckId
        ? navigationStateManager.buildUrl(location.pathname, currentParcours.id, effectiveCheckId)
        : `${location.pathname}?parcours=${currentParcours.id}`;

      console.log('🔗 CheckEasy: Mise à jour URL sans rechargement:', correctUrl);
      window.history.replaceState({}, '', correctUrl);
    }
  }, [parcoursInfo, currentParcours, currentCheckId, parcoursIdFromUrl, checkIdFromUrl, location.pathname]);

  // 🎯 FIX: Utiliser les données du parcours unifiées en priorité
  ...
  const correctedParcoursInfo = useMemo(() => {
    // Priorité: parcoursUnified (du hook) > parcoursInfo (du contexte global)
    let effectiveParcoursInfo = null;

    if (parcoursUnified?.adaptedData?.parcoursInfo) {
      // Utiliser les données du hook unifié (structure imbriquée)
      effectiveParcoursInfo = parcoursUnified.adaptedData.parcoursInfo;
    } else if (parcoursInfo) {
      // Fallback sur le contexte global
      effectiveParcoursInfo = parcoursInfo;
    }

    if (!effectiveParcoursInfo) return null;

    console.log('📋 CheckEasy: Données parcours utilisées:', {
      takePicture: effectiveParcoursInfo.takePicture,
      logement: effectiveParcoursInfo.logement,
      type: effectiveParcoursInfo.type,
      source: parcoursUnified ? 'unified' : 'global'
    });

    return effectiveParcoursInfo; // Utilisation directe des données API sans correction
  }, [parcoursInfo, parcoursUnified]);
  ...
  useEffect(() => {
    ...
    const ensureCheckIdExists = async () => {
      ...
      // 🆕 REFACTORISÉ: Utiliser checkIdFromUrl déjà extrait
      if (checkIdFromUrl) {
        return;
      }
  ...
  const [signalements] = useState<Signalement[]>([{
    ...
    piece: "Salle de bain",
    roomId: "salle_de_bain",
    origine: "Voyageur",
    commentaire: "Il y a une fuite d'eau importante sous le lavabo",
    status: "A_TRAITER",
    priorite: true,
    flowType: "checkin",
    ...
    commentaire: "Le réfrigérateur est complètement en panne",
    status: "A_TRAITER",
    priorite: false,
    flowType: "checkin",
  ...
  const mockHistoriqueSignalements: Record<string, Signalement[]> = {
    'salle_de_bain': [{
      ...
      roomId: "salle_de_bain",
      origine: "Agent Ménage",
      commentaire: "Les serviettes n'ont pas été changées",
      status: "RESOLU",
      priorite: false,
      flowType: "checkin",
    ...
    'chambre': [{
      ...
      roomId: "chambre",
      origine: "Voyageur",
      commentaire: "L'ampoule de la lampe de chevet ne fonctionne plus",
      status: "RESOLU",
      priorite: false,
      flowType: "checkin",
  ...
  const handleCompleteExit = () => {
    // 🆕 REFACTORISÉ: Utiliser navigationStateManager
    const effectiveParcoursId = parcoursIdFromUrl || currentParcours?.id;
    const effectiveCheckId = checkIdFromUrl || currentCheckId;

    const path = effectiveParcoursId && effectiveCheckId
      ? navigationStateManager.buildUrl('/checkout', effectiveParcoursId, effectiveCheckId)
      : '/checkout';
  ...
  const handleNavigateToSignalements = () => {
    ...
    const path = effectiveParcoursId && effectiveCheckId
      ? navigationStateManager.buildUrl('/signalements-a-traiter', effectiveParcoursId, effectiveCheckId)
      : '/signalements-a-traiter';
  ...
  // 🎯 FIX: Affichage d'erreur si configuration invalide
  // MAIS: Ne pas afficher l'erreur si le parcours est en cours de chargement
  // Cela évite une boucle infinie quand on crée une nouvelle session
  if (!validation.isValid && !loading && !parcoursUnifiedLoading) {
    return (
  ...
  // 🎯 FIX: Affichage d'un écran de chargement si le parcours est en cours de chargement
  if ((loading || parcoursUnifiedLoading) && !parcoursInfo) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
          ...
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {propertyData?.address || correctedParcoursInfo?.logement || 'Adresse non disponible'}
              </p>
                ...
                {(() => {
                  ...
                  if (user?.type === 'AGENT' || user?.type === 'GESTIONNAIRE') {
                    if (correctedParcoursInfo?.takePicture === 'checkOutOnly') {
                      ...
                      return 'Contrôle de l\'état du logement et validation du ménage';
                    }
                  }
                  // Pour les clients
                  if (correctedParcoursInfo?.takePicture === 'checkOutOnly') {
            ...
            <Card className="border border-border/50 cursor-pointer hover:bg-muted/50 transition-colors">
              <CardHeader className="py-3">
                ...
                <div className="grid grid-cols-2 gap-2 mt-3">
                  ...
                  {propertyData.visibleSections.includes('airbnb') && (
                    <Badge variant="outline" className="text-xs flex items-center gap-1.5 justify-start">
