...
import { CheckCircle2, AlertTriangle, Camera, X, Upload, ArrowLeft } from 'lucide-react';
...
export const EtatInitial = () => {
  ...
  const handleValidation = async () => {
    if (!canValidate) {
      ...
      if (missingPhotoRooms.length > 0) {
        toast.error('Au moins une photo est requise pour chaque pièce avec un problème signalé');
        return;
      }
    }
    ...
    if (deplorableRooms.length > 0 && user) {
      ...
      for (const roomState of deplorableRooms) {
        try {
          ...
          addSignalement({
            ...
            piece: room.nom,
            etapeId: undefined,
            titre: `Problème signalé - ${room.nom}`,
            commentaire: roomState.comment || 'Problème constaté lors du contrôle initial',
            imgUrl,
            imgBase64,
            flowType: 'checkin', // État initial = checkin
            origine: user.type,
            status: 'A_TRAITER',
            priorite: true, // Problème signalé = prioritaire
      ...
      toast.success(`État initial enregistré. ${deplorableRooms.length} pièce(s) avec problème(s) signalée(s).`);
    } else {
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            return (
              <Card key={room.id} variant="default" padding="default" className="transition-all duration-200">
                <div className="space-y-4">
                  ...
                  {/* Affichage des données du problème signalé */}
      ...
      {/* Popup pour signaler un problème */}
      <Dialog open={selectedRoomId !== null} onOpenChange={() => setSelectedRoomId(null)}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle>
              Signaler un problème - {selectedRoomId ? rooms.find(r => r.id === selectedRoomId)?.nom : ''}
            </DialogTitle>
          </DialogHeader>
