...
export const ExitQuestionsPage: React.FC<ExitQuestionsPageProps> = ({ questions }) => {
  ...
  const areAllQuestionsAnswered = (): boolean => {
    return questions.every(question => {
      ...
      // 🎯 NOUVEAU: Pour les questions de type "text", pas de validation (toujours optionnel)
      if (question.questionType === 'text') {
        return true; // Les questions texte sont toujours considérées comme valides
      }
      ...
      // 2. Pour les questions de type "image" : photo OBLIGATOIRE
      if (question.questionType === 'image') {
        if (!response.hasImage) return false;
      }

      // 3. Pour les questions avec imageRequired : photo OBLIGATOIRE
  ...
  return (
    <div className="min-h-screen bg-gradient-subtle max-w-md mx-auto relative">
      ...
      <div className="p-4 space-y-3 py-2 pb-32">
          ...
          >
            <CardContent className="p-3">
              <div className="flex items-start gap-2">
                <div className="flex-shrink-0 w-7 h-7 rounded-full bg-primary/20 flex items-center justify-center text-primary font-semibold text-xs">
                  {index + 1}
                </div>
