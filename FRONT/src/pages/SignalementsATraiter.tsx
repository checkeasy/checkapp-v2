...
export default function SignalementsATraiter() {
  ...
  const { currentParcours } = useGlobalParcours();  // ✅ NOUVEAU - Récupérer le parcours courant
  ...
  const allSignalements = useMemo(() => {
    ...
    // Combiner les signalements
    const combined = [...userSignalements, ...apiSignalementsATraiter];

    // ✅ Dédupliquer pour éviter les doublons (masquer les historiques qui ont une version utilisateur)
  ...
  const getOrigineBadgeColor = (origine: string) => {
    switch (origine.toLowerCase()) {
      ...
      case 'historique':  // ✅ NOUVEAU - Badge pour signalements API
  ...
  // ✅ NOUVEAU - Vérifier si un signalement est de l'API (read-only)
  const isApiSignalement = (signalement: Signalement) => {
    return signalement.origine === 'HISTORIQUE';
  };
            ...
            sortedSignalements.map((signalement) => (
              <Card key={signalement.id} className="border border-border/50">
                <CardContent className="p-4 space-y-3">
                  ...
                  {/* Actions */}
                  {isApiSignalement(signalement) ? (
                    // ✅ NOUVEAU - Signalements API sont en lecture seule
                    <div className="pt-2">
                      <div className="flex items-center gap-2 text-xs text-muted-foreground bg-purple-50 dark:bg-purple-950/20 p-2 rounded">
                        <History className="h-3 w-3" />
                        <span>Signalement historique (lecture seule)</span>
                      </div>
                    </div>
                  ) : (
                    // Signalements utilisateur avec actions
