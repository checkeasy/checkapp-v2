...
import { Phone, User, CheckCircle2, ArrowLeft, Briefcase, UserCheck, Download, Home, MapPin, Calendar as CalendarIcon } from 'lucide-react';
...
// 🌍 Configuration des indicatifs téléphoniques
...
const Welcome = () => {
  ...
  const [customCountryCode, setCustomCountryCode] = useState(''); // Pour "Autre"
  const [loadingProgress, setLoadingProgress] = useState(0); // Barre de chargement
  ...
  // 🎯 État pour les dates du séjour (voyageurs)
  const [dateRange, setDateRange] = useState<DateRange>({});
  const [isDatePickerOpen, setIsDatePickerOpen] = useState(false);

  // 🎯 Flag pour éviter de revérifier les sessions si l'utilisateur a choisi "Nouveau séjour"
  ...
  useEffect(() => {
    const restoreSessionFromUrl = async () => {
      ...
      login({
        ...
        phoneIndex: (userInfo as any).phoneIndex, // 🌍 NOUVEAU: Indicatif international
      ...
      // 🎯 FIX: Ajouter un délai pour permettre au contexte utilisateur de se mettre à jour
  ...
  useEffect(() => {
    ...
    const lastCountry = localStorage.getItem('lastCountryCode') as CountryCode | null;

    if (lastPhone) {
      // 🌍 Extraire le numéro sans l'index (ex: "+33 7 27 27 27 21" -> "7 27 27 27 21")
      // Chercher le premier espace après le "+" pour trouver où commence le numéro
      const phoneWithoutIndex = lastPhone.replace(/^\+\d+\s?/, '');
      setPhoneNumber(phoneWithoutIndex);
      console.log('✅ Welcome: Numéro de téléphone pré-rempli:', phoneWithoutIndex);
    }

    if (lastCountry && lastCountry in PHONE_COUNTRY_CODES) {
      setCountryCode(lastCountry);
      console.log('✅ Welcome: Indicatif pré-rempli:', lastCountry);
    }
  }, []);
  ...
  const saveUser = (phone: string, userData: StoredUser) => {
    ...
    // 🌍 Stocker avec le numéro COMPLET (avec index) pour différencier les pays
  ...
  const checkUserExists = (phone: string): StoredUser | null => {
    const users = getUsersStorage();
    // 🌍 Chercher avec le numéro COMPLET (avec index) pour différencier les pays
  ...
  const handlePhoneSubmit = async (e: React.FormEvent) => {
    ...
    // Vérifier l'indicatif personnalisé si "Autre" est sélectionné
    if (countryCode === 'OTHER' && (!customCountryCode.trim() || !customCountryCode.startsWith('+'))) {
      toast({
        title: "Indicatif requis",
        description: "Veuillez saisir un indicatif valide (ex: +212)",
        variant: "destructive"
      });
      return;
    }
    ...
    try {
      ...
      const finalCode = countryCode === 'OTHER' ? customCountryCode : PHONE_COUNTRY_CODES[countryCode].code;
      const fullPhoneNumber = `${finalCode}${phoneNumber}`;

      // 🌍 Vérifier l'utilisateur avec le numéro COMPLET (avec index) pour différencier les pays
      const user = checkUserExists(fullPhoneNumber);
      if (user) {
        setExistingUser(user);

        // 🎯 Vérifier si des sessions CheckID existent pour cet utilisateur et ce parcours
        // SAUF si l'utilisateur a déjà choisi de créer une nouvelle session
        if (parcoursId && !skipSessionCheck) {
          const hasExistingSession = await checkForExistingSessions(fullPhoneNumber, parcoursId);
        ...
        setUserInfo(prev => ({ ...prev, phone: fullPhoneNumber }));
      }

      // 💾 Sauvegarder le numéro complet et l'indicatif
      localStorage.setItem('lastUserPhone', fullPhoneNumber);
  ...
  const handleNewUserRegistration = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 🌍 Sauvegarder avec le numéro COMPLET (avec index) pour différencier les pays
      saveUser(fullPhoneNumber, {
        firstName: userInfo.firstName,
        lastName: userInfo.lastName,
  ...
  const createNewSessionAndLogin = async () => {
    ...
    // 🌍 Construire le numéro complet avec l'indicatif
    const phoneIndexValue = countryCode === 'OTHER' ? customCountryCode : PHONE_COUNTRY_CODES[countryCode].code;
    const fullPhoneNumber = `${phoneIndexValue}${phoneNumber}`;

    // ✅ Sauvegarder le numéro complet pour la prochaine fois
    localStorage.setItem('lastUserPhone', fullPhoneNumber);
    console.log('✅ Welcome: Numéro de téléphone sauvegardé:', fullPhoneNumber);
    ...
    if (!effectiveParcoursId || !globalParcoursInfo) {
      ...
      login({
        ...
        phoneIndex: phoneIndexValue, // 🌍 NOUVEAU: Indicatif international
        connectedAt: new Date().toISOString(),
        type: userType
      });
    ...
    try {
      ...
      const newCheckId = await createNewCheckId(
        {
          ...
          lastName: currentUser.lastName,
          phone: fullPhoneNumber,
          phoneIndex: phoneIndexValue, // 🌍 NOUVEAU: Ajouter l'indicatif international
          type: userType
        },
        {
          ...
          checkinDate: dateRange.from?.toISOString() || null,
          checkoutDate: dateRange.to?.toISOString() || null
        },
        flowType
      );
      ...
      // 🎯 FIX: Ajouter un délai pour permettre au contexte utilisateur de se mettre à jour
      // Cela évite que le ProtectedRoute redirige vers /welcome avant que isAuthenticated soit true
      await new Promise(resolve => setTimeout(resolve, 100));
  ...
  const handleCreateNewSession = async () => {
    ...
    // 🎯 FIX: Activer le flag pour éviter de revérifier les sessions
    setSkipSessionCheck(true);

    // 🌍 NOUVEAU: Pour les voyageurs (CLIENT), afficher le sélecteur de dates
    if (userType === 'CLIENT') {
      // Réinitialiser les dates (SANS ouvrir le calendrier automatiquement)
      setDateRange({});
      setIsDatePickerOpen(false); // ✅ FIX: Ne pas ouvrir automatiquement
      setStep(2); // Aller à l'étape 2 pour afficher le sélecteur de dates

      // 🎯 Récupérer les infos de l'utilisateur depuis la première session active
      if (userSessionsList && userSessionsList.activeSessions.length > 0) {
        const firstSession = userSessionsList.activeSessions[0];
        if (firstSession.userInfo) {
          setExistingUser({
            firstName: firstSession.userInfo.firstName,
            lastName: firstSession.userInfo.lastName,
            registeredAt: firstSession.createdAt
          });
          console.log('✅ Welcome: Infos utilisateur récupérées depuis la session:', firstSession.userInfo);
        }
      }
      return;
    }

    // Pour les agents/gestionnaires, créer directement la session
    setIsLoading(true);
    ...
    const progressInterval = setInterval(() => {
      setLoadingProgress(prev => {
        if (prev >= 90) return prev; // Stop à 90%, complète à la fin
    ...
    try {
      await createNewSessionAndLogin();
      setLoadingProgress(100); // Complète à 100%
    } catch (error) {
      toast({
        title: "Erreur",
  ...
  const handleResumeSpecificSession = async (session: CheckSession) => {
    ...
    try {
      ...
      login({
        ...
        phoneIndex: (userInfoToUse as any).phoneIndex, // 🌍 NOUVEAU: Indicatif international
        connectedAt: new Date().toISOString(),
    ...
    <div className="min-h-screen bg-background flex flex-col">
      ...
      {showInstallButton && (
        <div className="absolute top-3 right-3 sm:top-4 sm:right-4 z-10">
          <Button
            ...
            variant="outline"
            size="sm"
            className="flex items-center gap-2 shadow-md hover:shadow-lg transition-all bg-background/90 backdrop-blur-sm text-xs sm:text-sm"
          >
      ...
      {isLoading && loadingProgress > 0 && (
        <div className="fixed top-0 left-0 right-0 z-50">
          <Progress value={Math.min(loadingProgress, 100)} className="h-1 rounded-none" />
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 py-6 pb-32">
        <div className="w-full max-w-sm mx-auto space-y-4 sm:space-y-6">
          ...
          <div className="text-center space-y-2 sm:space-y-3">
            <div>
              {step === 1 ? (
                ...
                <>
                  <h1 className="text-xl sm:text-2xl font-bold text-foreground">Bonjour {existingUser.firstName} !</h1>
                  {parcoursInfo && (
                    <div className="mt-2">
                      <div className="flex items-center justify-center gap-2">
                        <MapPin className="h-4 w-4 text-primary" />
          ...
          <Card className="p-4 sm:p-6 shadow-lg border-0 bg-card/80 backdrop-blur-sm">
            {/* 🎯 MASQUÉ: Affichage des informations du parcours
            {parcoursInfo && step === 1 && (
              <div className="mb-4 sm:mb-6 p-3 sm:p-4 bg-primary/5 border border-primary/10 rounded-lg">
            ...
            */}
            ...
            {step === 1 ? (
              ...
              <div className="space-y-4 sm:space-y-5">
                ...
                */}
                ...
                <div className="space-y-2">
                  ...
                  {/* Sélecteur de pays + Indicatif personnalisé */}
                  <div className="flex gap-2">
                    <div className="flex items-center gap-0.5 h-11 sm:h-12 px-2.5 sm:px-3 rounded-md border border-input bg-background w-fit">
                      <select
                        value={countryCode}
                        onChange={(e) => setCountryCode(e.target.value as CountryCode)}
                        className="bg-transparent text-sm sm:text-base font-medium text-foreground focus:outline-none appearance-none cursor-pointer"
                      >
                        {Object.entries(PHONE_COUNTRY_CODES).map(([key, { flag, code }]) => (
                      ...
                      {/* Champ indicatif personnalisé - Inline si "Autre" est sélectionné */}
                      {countryCode === 'OTHER' && (
                        <input
                          type="text"
                          inputMode="numeric"
                          ...
                          maxLength={3}
                          className="w-10 sm:w-12 bg-transparent text-sm sm:text-base font-medium text-foreground focus:outline-none text-center"
                        />
                      )}
                    </div>
                    ...
                    <div className="relative flex-1">
                      <Input
                        ...
                        value={phoneNumber.replace(/(\d)(\d{2})(\d{2})(\d{2})(\d{2})/, '$1 $2 $3 $4 $5').trim()}
                        onChange={(e) => {
                        ...
                        maxLength={14}
                        className="bg-background/60 border-border/40 focus:border-primary text-sm"
                      />
                    </div>
                  </div>
                </div>
              ...
              <div className="space-y-5">
                <div className="text-center p-4 bg-primary/5 rounded-lg border border-primary/10">
                  <p className="text-sm text-muted-foreground">
                    Numéro: <span className="font-medium text-foreground">{countryCode === 'OTHER' ? customCountryCode : PHONE_COUNTRY_CODES[countryCode].code} {phoneNumber.replace(/(\d)(\d{2})(\d{2})(\d{2})(\d{2})/, '$1 $2 $3 $4 $5').trim()}</span>
                  </p>
                </div>
                ...
                <Button
                  onClick={handleBack}
                  variant="ghost"
                  className="w-fit h-auto px-0 py-1 text-xs sm:text-sm text-secondary hover:text-secondary hover:bg-transparent"
                >
                  <ArrowLeft className="h-3 w-3 mr-1" />
                  Ce n'est pas moi
                </Button>

                {/* 🎯 Sélecteur de dates pour les voyageurs existants */}
                {userType === 'CLIENT' && (
                  <div className="space-y-2">
                    ...
                    <Popover open={isDatePickerOpen} onOpenChange={(open) => {
                      ...
                      // Réinitialiser complètement la sélection quand on ouvre le calendrier
                      if (open) {
                        setDateRange({});
                      }
                    }}>
                      <PopoverTrigger asChild>
                        <Button
                          variant="outline"
                          className="w-full justify-start text-left font-normal bg-background/60 border-border/40 hover:bg-background/80 hover:border-primary/50 h-11 sm:h-12"
                        >
                          ...
                          {dateRange.from && dateRange.to ? (
                            ...
                            <span className="text-muted-foreground">Sélectionner les dates de séjour</span>
                          )}
                        </Button>
                      </PopoverTrigger>
              ...
              <div className="space-y-4 sm:space-y-5">
                <div className="text-center p-2.5 sm:p-3 bg-primary/5 rounded-lg border border-primary/10">
                  <p className="text-xs text-muted-foreground">
                    Numéro: <span className="font-medium text-foreground text-xs sm:text-sm">{countryCode === 'OTHER' ? customCountryCode : PHONE_COUNTRY_CODES[countryCode].code} {phoneNumber.replace(/(\d)(\d{2})(\d{2})(\d{2})(\d{2})/, '$1 $2 $3 $4 $5').trim()}</span>
                  </p>
                </div>
                ...
                <div className="space-y-3 sm:space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input
                        ...
                        className="pl-10 bg-background/60 border-border/40 focus:border-primary text-sm"
                      />
                    </div>
                  </div>
                  ...
                  {/* 🎯 Sélecteur de dates pour les voyageurs */}
                  {userType === 'CLIENT' && (
                    <div className="space-y-2">
                      <Label className="text-xs sm:text-sm font-medium text-foreground">
                        Dates du séjour
      ...
      {/* CTA collant en bas */}
      <div className="fixed bottom-0 left-0 right-0 p-3 sm:p-4 bg-gradient-to-t from-background via-background/95 to-transparent backdrop-blur-sm border-t border-border/20 pb-safe pb-6 sm:pb-8">
          ...
          {step === 1 ? (
            <div className="space-y-2 sm:space-y-3">
              <Button
                ...
                className="w-full h-12 sm:h-14 bg-primary hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70 text-primary-foreground font-medium text-sm sm:text-base shadow-lg hover:shadow-xl transition-all duration-200"
              >
                {parcoursLoading ? (
              ...
              <p className="text-xs text-center text-muted-foreground px-2 leading-snug">
            ...
            <Button
              ...
              disabled={isLoading || (userType === 'CLIENT' && (!dateRange.from || !dateRange.to))}
              className="w-full h-12 sm:h-14 bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-sm sm:text-base shadow-lg hover:shadow-xl transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
                <div className="flex items-center space-x-2">
                  <div className="w-4 h-4 border-2 border-primary-foreground/30 border-t-primary-foreground rounded-full animate-spin" />
            ...
            <div className="space-y-2 sm:space-y-3">
              <Button
                ...
                disabled={isLoading || !userInfo.firstName.trim() || !userInfo.lastName.trim() || (userType === 'CLIENT' && (!dateRange.from || !dateRange.to))}
                className="w-full h-12 sm:h-14 bg-primary hover:bg-primary/90 disabled:bg-muted disabled:text-muted-foreground disabled:opacity-70 text-primary-foreground font-medium text-sm sm:text-base shadow-lg hover:shadow-xl transition-all duration-200"
              >
                {isLoading ? (
                  <div className="flex items-center space-x-2">
      ...
      {userSessionsList && (
        <UserSessionsListDialog
          ...
          onClose={() => {
            setShowUserSessionsList(false);
            // 🎯 FIX: Réinitialiser le flag quand on ferme la modal
