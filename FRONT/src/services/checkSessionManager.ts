...
class CheckSessionManager {
  ...
  /**
   * 🏁 Marque une session comme terminée (après envoi du webhook final)
   * et sauvegarde l'ID du rapport Bubble
   */
  async terminateCheckSession(checkId: string, rapportID?: string): Promise<void> {
    try {
      const session = await this.getCheckSession(checkId);
      if (!session) {
        console.error('❌ Session non trouvée:', checkId);
      ...
      // 🆕 Stocker completedAt si pas déjà fait
      if (!session.completedAt) {
        session.completedAt = new Date().toISOString();
        console.log('✅ completedAt stocké:', session.completedAt);
      }

      if (rapportID) {
        session.rapportID = rapportID;
        console.log('📋 Rapport ID sauvegardé:', rapportID);
      }

      await this.saveCheckSession(session);
      console.log('🏁 Session terminée:', checkId);
    } catch (error) {
      console.error('❌ Erreur terminateCheckSession:', error);
      throw error;
    }
  }
  ...
  /**
   * 🧹 Vide TOUTES les sessions de IndexedDB
   * Utilisé lors de la déconnexion pour nettoyer complètement
   */
  async clearAllSessions(): Promise<void> {
    try {
      ...
      await new Promise<void>((resolve, reject) => {
        ...
        request.onerror = () => {
          console.error('❌ Erreur suppression sessions:', request.error);
          reject(request.error);
        };
      });
    } catch (error) {
      console.error('❌ Erreur clearAllSessions:', error);
      throw error;
    }
  }
}
