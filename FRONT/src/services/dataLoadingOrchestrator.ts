...
class DataLoadingOrchestrator {
  ...
  private async _loadParcoursDataInternal(
    ...
    try {
      const strategy = this.cacheStrategy.strategy;
      console.log(`🔍 [DataLoadingOrchestrator] Stratégie: ${strategy}`);

      // Stratégie: network-only
      if (strategy === 'network-only') {
        console.log(`🌐 [DataLoadingOrchestrator] Chargement parcours ${parcoursId} depuis l'API (network-only)...`);
      ...
      const cacheValid = cachedData && await parcoursCache.isCacheValid(parcoursId, this.cacheStrategy.maxAgeHours);
      console.log(`🔍 [DataLoadingOrchestrator] Cache valide:`, cacheValid);

      // Stratégie: cache-only
      if (strategy === 'cache-only') {
        if (cacheValid && cachedData) {
          console.log(`✅ [DataLoadingOrchestrator] Parcours ${parcoursId} chargé depuis le cache (cache-only)`);
          return parcoursManager.loadFromRawDataWithMode(cachedData, forceFlowType);
        } else {
          throw new Error(`Cache non disponible pour parcours ${parcoursId} (cache-only mode)`);
        }
      }

      // Stratégie: cache-first
      if (strategy === 'cache-first' && cacheValid && cachedData) {
        ...
        // Recharger en arrière-plan si activé
        if (this.cacheStrategy.enableBackgroundRevalidation) {
      ...
      // Stratégie: network-first ou cache-first sans cache valide
      ...
      try {
        ...
        // Si network-first et erreur API, fallback sur cache
        if (strategy === 'network-first' && cachedData) {
          console.warn(`⚠️ [DataLoadingOrchestrator] Erreur API, fallback sur cache:`, apiError);
          return parcoursManager.loadFromRawDataWithMode(cachedData, forceFlowType);
        }
        throw apiError;
      }
   ...
   */
  clearCache(): void {
    console.log('🧹 [DataLoadingOrchestrator] Nettoyage du cache');
    try {
      parcoursCache.clearAllCache();
      this.clearAllLocks();
