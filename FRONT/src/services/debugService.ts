...
export interface DebugResult {
  ...
  timestamp: string;
  error?: string;
  payloadType?: 'enriched' | 'unified_v2';
  rapportID?: string;  // 🎯 ID du rapport Bubble (récupéré depuis la réponse webhook)
}
...
class DebugService {
  ...
  private buildPieceEtapes(
    ...
    donneesUtilisateur.photos
      ...
      .forEach((photo: any) => {
        // 🔒 SÉCURITÉ: Ne JAMAIS mettre une URL dans photo_base64
        ...
        etapes.push({
          ...
          photo_url: isUrl ? photoUrl : '',           // URL uniquement si c'est vraiment une URL
          photo_base64: isBase64 ? photoUrl : '',     // Base64 uniquement si c'est vraiment une base64
      ...
      if (activeSession?.progress?.interactions?.signalements) {
        ...
        Object.entries(sessionSignalements).forEach(([key, sig]: [string, any]) => {
          const signalement = {
            ...
            rapportRef: sig.metadata?.rapportRef || sig.rapportRef // ✅ NOUVEAU - Charger rapportRef
          };
  ...
  async sendUnifiedWebhook(customUrl?: string): Promise<DebugResult> {
    try {
      ...
      // 🔍 DEBUG APPROFONDI: Afficher le JSON COMPLET du payload (avec base64 tronqué)
      console.log('📋 ========== PAYLOAD JSON COMPLET ==========');

      // Fonction pour tronquer les base64 dans le JSON
      const truncateBase64 = (obj: any): any => {
        if (obj === null || obj === undefined) return obj;

        if (typeof obj === 'string') {
          // Si c'est une chaîne base64 (commence par data:image ou très longue)
          if (obj.startsWith('data:image') || (obj.length > 100 && /^[A-Za-z0-9+/=]+$/.test(obj))) {
            return obj.substring(0, 50) + '... [TRONQUÉ - ' + obj.length + ' caractères]';
          }
          return obj;
        }
        ...
        if (typeof obj === 'object') {
          ...
          for (const key in obj) {
            // Tronquer spécifiquement les champs base64
            if (key === 'photo_base64' || key === 'img_base64' || key === 'photoBase64' || key === 'dataUrl') {
              if (obj[key] && typeof obj[key] === 'string') {
                result[key] = obj[key].substring(0, 50) + '... [BASE64 TRONQUÉ - ' + obj[key].length + ' caractères]';
              } else {
                result[key] = obj[key];
              }
            } else {
      ...
      const payloadWithTruncatedBase64 = truncateBase64(webhookPayload);
      console.log(JSON.stringify(payloadWithTruncatedBase64, null, 2));
      console.log('📋 ========== FIN PAYLOAD JSON ==========');

      // 🔍 DEBUG: Afficher la structure détaillée
      console.log('🔍 Structure détaillée du payload:', {
        webhook_version: webhookPayload.webhook_version,
        schema: webhookPayload.schema,
        checkID: webhookPayload.checkID,
        parcours_id: webhookPayload.parcours_id,
        agent: webhookPayload.agent,
        parcours: webhookPayload.parcours,
        checkin_pieces_count: webhookPayload.checkin?.pieces?.length || 0,
        checkin_first_piece: webhookPayload.checkin?.pieces?.[0] || null,
        checkout: webhookPayload.checkout,
        signalements_count: webhookPayload.signalements?.length || 0,
        exit_questions_count: webhookPayload.exit_questions?.length || 0,
  ...
  private async sendToWebhookEndpoint(payload: EnrichedAPIPayload, url: string, type: string): Promise<DebugResult> {
    ...
    try {
      ...
      // 🎯 Extraire le rapportID depuis la réponse Bubble
      let rapportID: string | undefined;
      if (result?.response?.rapportID?._id) {
        rapportID = result.response.rapportID._id;
        console.log('📋 Rapport ID récupéré depuis Bubble:', rapportID);
      }

      // ✅ Déterminer le vrai type de payload
      const actualPayloadType = isNewFormat ? 'unified_v2' : 'enriched';

      return {
        success: true,
        debugId: result.debugId || result.id || 'unknown',
        timestamp: new Date().toISOString(),
        payloadType: actualPayloadType as any,
        rapportID  // 🎯 Ajouter le rapportID à la réponse
      };
