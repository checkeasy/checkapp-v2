...
class ParcoursManager {
  ...
  async loadParcours(parcoursId: string, forceFlowType?: FlowType): Promise<ParcoursData> {
    ...
    try {
      // Construire l'URL de l'API avec le bon endpoint (depuis environment.ts)
      const apiUrl = `${environment.PARCOURS_DATA_URL}?parcours=${parcoursId}`;
      ...
      // 🎯 DEBUG: Log détaillé des pièces et leurs notes
      if (rawData.piece && Array.isArray(rawData.piece)) {
        console.log('📋 Nombre de pièces dans rawData:', rawData.piece.length);
        rawData.piece.forEach((piece: any, index: number) => {
          console.log(`🏠 Pièce ${index + 1}/${rawData.piece.length} - "${piece.nom}":`, {
            pieceID: piece.pieceID,
            travelerNote: piece.travelerNote || '(vide)',
            cleanerNote: piece.cleanerNote || '(vide)',
            infoEntrance: piece.infoEntrance || '(vide)',
            hasTravelerNote: !!piece.travelerNote,
            hasCleanerNote: !!piece.cleanerNote,
            hasInfoEntrance: !!piece.infoEntrance
          });
        });
      }
