...
class UrlPersistenceService {
  ...
  saveCurrentUrlParams(): void {
    try {
      ...
      let checkid = urlParams.get('checkid');
      const path = window.location.pathname;

      // 🆕 FIX: Si checkid n'est pas dans l'URL, essayer de le récupérer depuis localStorage
      if (!checkid) {
        const activeCheckId = localStorage.getItem('activeCheckId');
        if (activeCheckId) {
          checkid = activeCheckId;
          console.log('💡 Using activeCheckId from localStorage:', activeCheckId);
        }
      }
   ...
   *
   * 🎯 NEW LOGIC:
   * - If URL has ONLY parcours (no checkid) → User wants NEW session → Do NOT restore checkid
   * - If URL has parcours + checkid → User wants to RESTORE session → Restore saved path
   * - Only restore checkid if URL already contains one (to restore other params)
   */
  restoreUrlParamsIfMissing(): boolean {
    try {
      ...
      // 🎯 CRITICAL FIX: If URL has parcours but NO checkid, user wants a NEW session
      // Do NOT restore checkid automatically
      if (currentParcours && !currentCheckid) {
        console.log('🆕 NEW SESSION DETECTED: URL has parcours but no checkid');
        console.log('   → User wants to start a fresh session, NOT restoring checkid');
        // Save current params (parcours only) for this new session
      ...
      // Restore parcours if missing (only when URL has NO params at all)
      ...
      // 🎯 CRITICAL FIX: Only restore checkid if we're restoring parcours too
      // This means URL had NO params at all (page refresh on a non-welcome page)
      // Never restore checkid if URL already has parcours (user wants new session)
      if (!currentParcours && !currentCheckid && restored) {
