...
export interface WebhookPayload {
  ...
  agent: {
    ...
    phone: string | null;
    phoneIndex?: string | null; // 🌍 NOUVEAU: Indicatif international (ex: "+33", "+41", etc.)
    type: string;
    type_label: string;
...
export async function generateUnifiedWebhookData(
  ...
  const payload: WebhookPayload = {
    ...
    agent: {
      ...
      phoneIndex: userVerificationData.user_info?.phoneIndex || null, // 🌍 NOUVEAU: Indicatif international
...
function extractUserVerificationData(sessionData: SessionData) {
  ...
  if (sessionData.userInfo) {
    userVerificationData.user_info = {
      ...
      phoneIndex: sessionData.userInfo.phoneIndex || null, // 🌍 NOUVEAU: Indicatif international
...
/**
 * 🎯 Extraire les questions de sortie
 * ⚠️ IMPORTANT: Cette fonction doit rester EXACTEMENT synchronisée avec database-admin.html (lignes 1293-1331)
 */
async function extractExitQuestions(sessionData: SessionData): Promise<any[]> {
  ...
  // Récupérer les réponses depuis sessionData.progress.interactions.exitQuestions
    ...
    Object.entries(sessionData.progress.interactions.buttonClicks).forEach(([key, clickArray]: [string, any]) => {
      ...
      if (belongsToPiece && Array.isArray(clickArray)) {
        clickArray.forEach((click: any) => {
          ...
          // 🎯 FIX: Extraire le vrai etapeId (sans le pieceId)
          let rawEtapeId = null;

          // PRIORITÉ 1: metadata.etapeId (le plus fiable)
          if (click.metadata?.etapeId && /^\d+x\d+$/.test(click.metadata.etapeId)) {
            rawEtapeId = click.metadata.etapeId;
          }
          // PRIORITÉ 2: click.etapeId
          else if (click.etapeId && /^\d+x\d+$/.test(click.etapeId)) {
            rawEtapeId = click.etapeId;
          }
          // PRIORITÉ 3: click.buttonId
          else if (click.buttonId && /^\d+x\d+$/.test(click.buttonId)) {
            rawEtapeId = click.buttonId;
          }
          // PRIORITÉ 4: Extraire depuis taskId si format "pieceId_etapeId"
          else if (click.taskId) {
            if (/^\d+x\d+$/.test(click.taskId)) {
              // taskId est déjà au bon format
              rawEtapeId = click.taskId;
            } else if (click.taskId.includes('_')) {
              // taskId contient "pieceId_etapeId", extraire l'etapeId
              const parts = click.taskId.split('_');
              const extractedId = parts[parts.length - 1];
              if (/^\d+x\d+$/.test(extractedId)) {
                rawEtapeId = extractedId;
              }
            }
          }
          ...
          console.log('🔍 Extraction etapeId pour button click:', {
            buttonId: click.buttonId,
            taskId: click.taskId,
            'click.etapeId': click.etapeId,
            'metadata.etapeId': click.metadata?.etapeId,
            extractedEtapeId: realEtapeId
          });
    ...
    Object.entries(sessionData.progress.interactions.photosTaken).forEach(([photoKey, photoArray]: [string, any]) => {
      if (photoArray && Array.isArray(photoArray)) {
        photoArray.forEach((photo: any) => {
          ...
          // 🎯 FIX: Extraire l'etapeId en priorité depuis metadata.etapeId
          let photoEtapeId = null;

          // PRIORITÉ 1: Utiliser metadata.etapeId si disponible (le plus fiable)
          if (photo.metadata?.etapeId && /^\d+x\d+$/.test(photo.metadata.etapeId)) {
            photoEtapeId = photo.metadata.etapeId;
          }

          // PRIORITÉ 2: Extraire depuis taskId si format "pieceId_etapeId"
          if (!photoEtapeId && photo.taskId && photo.taskId.includes('_')) {
            const parts = photo.taskId.split('_');
            const extractedId = parts[parts.length - 1];
            if (/^\d+x\d+$/.test(extractedId)) {
              photoEtapeId = extractedId;
            }
          }

          // PRIORITÉ 3: Utiliser photo.etapeId directement
          if (!photoEtapeId && photo.etapeId && /^\d+x\d+$/.test(photo.etapeId)) {
            photoEtapeId = photo.etapeId;
          }

          // FALLBACK: Utiliser photoId si rien d'autre ne fonctionne
          if (!photoEtapeId) {
            photoEtapeId = photo.metadata?.photoId || photo.photoId || `photo-${photo.photoId}`;
          }

          console.log('🔍 Extraction etapeId pour photo:', {
            photoId: photo.photoId,
            taskId: photo.taskId,
            'metadata.etapeId': photo.metadata?.etapeId,
            'photo.etapeId': photo.etapeId,
            extractedEtapeId: photoEtapeId
          });

          // 🔒 SÉCURITÉ: Séparer correctement URL et Base64
          const photoDataValue = photo.photoData || photo.metadata?.url || photo.uploadedUrl || '';
          const separated = separateUrlAndBase64(photoDataValue);

          const photoData = {
            photo_id: photo.photoId,
            url: separated.url,  // URL uniquement si c'est une URL
          ...
          const photoEtape = {
            ...
            comment: '',
            photo_id: photo.photoId,
            // 🔒 SÉCURITÉ: Utiliser la séparation sécurisée
            photo_base64: separated.base64,  // Base64 uniquement si c'est vraiment une base64
            photo_url: separated.url,        // URL uniquement si c'est vraiment une URL
            validated: photo.validated || false,
            retake_count: photo.retakeCount || 0,
  ...
  if (sessionData.progress.interactions.checkboxStates) {
    Object.entries(sessionData.progress.interactions.checkboxStates).forEach(([checkboxKey, checkboxData]: [string, any]) => {
      ...
      if (checkboxPieceId === pieceId || checkboxKey.includes(pieceId)) {
        ...
        // 🎯 FIX: Extraire le vrai etapeID (sans le pieceId)
        let checkboxEtapeId = null;

        // PRIORITÉ 1: Utiliser metadata.etapeId si disponible
        if (checkboxData.metadata?.etapeId && /^\d+x\d+$/.test(checkboxData.metadata.etapeId)) {
          checkboxEtapeId = checkboxData.metadata.etapeId;
        }

        // PRIORITÉ 2: Utiliser etapeId directement
        if (!checkboxEtapeId && checkboxData.etapeId && /^\d+x\d+$/.test(checkboxData.etapeId)) {
          checkboxEtapeId = checkboxData.etapeId;
        }

        // PRIORITÉ 3: Extraire depuis taskId si format "pieceId_etapeId"
        if (!checkboxEtapeId && checkboxData.taskId && checkboxData.taskId.includes('_')) {
          const parts = checkboxData.taskId.split('_');
          const extractedId = parts[parts.length - 1];
          if (/^\d+x\d+$/.test(extractedId)) {
            checkboxEtapeId = extractedId;
          }
        }

        // FALLBACK: Utiliser taskId ou checkboxKey
        if (!checkboxEtapeId) {
          checkboxEtapeId = checkboxData.taskId || checkboxKey;
        }

        console.log('🔍 Extraction etapeId pour checkbox:', {
          checkboxKey,
          taskId: checkboxData.taskId,
          'metadata.etapeId': checkboxData.metadata?.etapeId,
          'checkboxData.etapeId': checkboxData.etapeId,
          extractedEtapeId: checkboxEtapeId
        });
